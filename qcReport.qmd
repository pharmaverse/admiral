...
for (y in input_dataset_names) {
  ...
  cat("\n\n```\n\n", file="qcSummary.html", append = TRUE)
  x =print(diffs, as_string = TRUE)
  cat(x, file="qcSummary.html", append = TRUE) 
  cat("```\n\n", file="qcSummary.html", append = TRUE)
  cat("</details>", file="qcSummary.html", append = TRUE)
  cat("\n\n", file="qcSummary.html", append = TRUE)
}
```
...
# Neither cat nor writeLines can  accept cli_h1, use html markup symbols
#
...
cat("Run by: ", Sys.getenv("GITHUB_ACTOR"), "\n")
cat("Git Ref: ", Sys.getenv("GITHUB_REF"), "\n")
cat("BASE: ", "Generated ADaM Datasets from Templates ", "\n")
cat("COMPARE: ", "ADaM Datasets from pharmaverseadam ", "\n")
for (y in input_dataset_names) {
