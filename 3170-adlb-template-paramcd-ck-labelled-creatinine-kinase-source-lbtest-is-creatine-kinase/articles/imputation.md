...
briefly explores the imputation rules used in
[admiral](https://pharmaverse.github.io/admiral/). The second section
focuses on imputation functions that work on vectors with lots of small
...
partials then these functions should be used as a first step to make the
required imputation.
...
this rule allows a user to control which components of the `--DTC` value
are imputed if they are missing.

The default imputation for `_dtm()` functions,
e.g. [`impute_dtc_dtm()`](https:/pharmaverse.github.io/admiral/3170-adlb-template-paramcd-ck-labelled-creatinine-kinase-source-lbtest-is-creatine-kinase/reference/impute_dtc_dtm.md),
[`derive_vars_dtm()`](https:/pharmaverse.github.io/admiral/3170-adlb-template-paramcd-ck-labelled-creatinine-kinase-source-lbtest-is-creatine-kinase/reference/derive_vars_dtm.md),
...
e.g. [`impute_dtc_dt()`](https:/pharmaverse.github.io/admiral/3170-adlb-template-paramcd-ck-labelled-creatinine-kinase-source-lbtest-is-creatine-kinase/reference/impute_dtc_dt.md),
[`derive_vars_dt()`](https:/pharmaverse.github.io/admiral/3170-adlb-template-paramcd-ck-labelled-creatinine-kinase-source-lbtest-is-creatine-kinase/reference/derive_vars_dt.md)
...
is not desirable. The [admiral](https://pharmaverse.github.io/admiral/)
imputation functions provide the `min_dates` and `max_dates` argument to
specify those dates. For example:
...
dates. Only dates which are in the range of possible dates of the
`--DTC` value are considered. The possible dates are defined by the
missing parts of the `--DTC` date, i.e., for “2019-02” the possible
dates range from “2019-02-01” to “2019-02-28”. Thus “2019-01-14” is
ignored. This ensures that the non-missing parts of the `--DTC` date are
not changed.
...
[admiral](https://pharmaverse.github.io/admiral/) functions which derive
imputed variables are also adding the corresponding imputation flag
variables.

Note: The [admiral](https://pharmaverse.github.io/admiral/) datetime
imputation function provides the `ignore_seconds_flag` argument which
can be set to `FALSE` in cases where seconds are expected. This is due
to the following from ADaM IG: For a given SDTM `--DTC` variable, if
...
representations of the same date. Thus the imputation must be the same
and the imputation flags are valid for both the datetime and the date
variable.
