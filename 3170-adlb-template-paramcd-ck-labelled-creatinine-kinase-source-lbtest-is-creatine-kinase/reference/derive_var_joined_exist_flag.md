...
- dataset:
  ...
  The variables specified by the `by_vars` and `join_vars` arguments are
...
- by_vars:
  ...
  (`dataset`) with the additional dataset (`dataset_add`).
...
- order:
  ...
  `join_type = "after"`, `join_type = "before"`, `first_cond_lower`,
  `first_cond_upper`, or `tmp_obs_nr_var` are specified.

  For handling of `NA`s in sorting variables see the "Sort Order"
  section in
  [`vignette("generic")`](https:/pharmaverse.github.io/admiral/3170-adlb-template-paramcd-ck-labelled-creatinine-kinase-source-lbtest-is-creatine-kinase/articles/generic.md).
...
- tmp_obs_nr_var:
  ...
  included in the output dataset. It can also be used to flag
  consecutive observations or the last observation (see last example
  below).
...
- join_vars:
  ...
  dataset with suffix ".join". For example to flag all observations with
  `AVALC == "Y"` and `AVALC == "Y"` for at least one subsequent visit
...
- first_cond_upper:
  ...
  the confirmation assessment. For an example see the third example
  below.
...
In the oncology setting, for example, the function could be used to flag
if a response value can be confirmed by an other assessment. This is
...
dataset only the variables specified for `join_vars` are kept. The
suffix ".join" is added to those variables which also exist in the input
dataset.
...
specified by the `filter_join` argument. Variables from the other
records are referenced by variable names with the suffix `.join`. These
variables have to be specified for the `join_vars` argument. As records
before *and* after the current record should be considered,
...
In this example, records with `AVALC == "Y"` and `AVALC == "Y"` at a
subsequent visit should be flagged. `join_type = "after"` is specified
to consider only records after the current one. Please note that the
`order` argument must be specified, as otherwise it is not possible to
...
To flag `"0"` where all results from the first `"++"` before the `"0"`
...
To flag `"0"` where all results from the `"0"` (excluding the `"0"`) up
...
### Considering order of values ([`min_cond()`](https:/pharmaverse.github.io/admiral/3170-adlb-template-paramcd-ck-labelled-creatinine-kinase-source-lbtest-is-creatine-kinase/reference/min_cond.md), [`max_cond()`](https:/pharmaverse.github.io/admiral/3170-adlb-template-paramcd-ck-labelled-creatinine-kinase-source-lbtest-is-creatine-kinase/reference/max_cond.md))
...
visits or at the last visit should be flagged. A temporary order
...
In this example a definitive deterioration flag should be derived as any
deterioration (`CHGCAT1 = "Worsened"`) by parameter that is not followed
by a non-deterioration. Please note that `join_type = "after"` can't by
used here, as otherwise the last record wouldn't be flagged.
...
If the `order` argument is used, it is checked if the records are unique
with respect to `by_vars` and `order`. Consider for example the
...
flagged. Thus the warning can be suppressed by specifying
`check_type = "none"`.
