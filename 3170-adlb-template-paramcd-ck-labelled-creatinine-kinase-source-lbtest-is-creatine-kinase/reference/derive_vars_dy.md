...
the end date. If it is nonnegative, one is added. That is, the relative
day of the reference date is 1. Unless a name is explicitly specified,
the name of the resulting relative day variable is generated from the
source variable name by replacing DT (or DTM as appropriate) with DY. In
the ADaM as in the SDTM, there is no Day 0. If there is a need to create
a relative day variable that includes Day 0, then its name must not end
in DY.
