...
adpc <- adpc %>%
  derive_vars_joined(
    ...
    order = vars(ADTM),
    new_vars = vars(ADTM_prev = ADTM, EXDOSE_prev = EXDOSE, AVISIT_prev = AVISIT, AENDTM_prev = AENDTM),
    join_vars = vars(ADTM),
    filter_add = NULL,
...
adpc <- adpc %>%
  derive_vars_joined(
    ...
    by_vars = vars(USUBJID),
    order = vars(ADTM),
    new_vars = vars(ADTM_next = ADTM, EXDOSE_next = EXDOSE, AVISIT_next = AVISIT, AENDTM_next = AENDTM),
    join_vars = vars(ADTM),
    filter_add = NULL,
    filter_join = ADTM <= ADTM.join,
    mode = "first",
