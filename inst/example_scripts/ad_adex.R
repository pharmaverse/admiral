...
param_lookup <- tibble::tribble(
  ...
  "TDOSE", "Total dose administered", 8,
  "ADOSE", "Average dose administered", 9,
  "AVDOSE", "Average daily dose administered", 10,
...
tpdose <- exposure_var_spec(
  new_parameter = "TPDOSE",
  input_parameter = "PLDOSE",
  fns = list(AVAL ~ sum(., na.rm = TRUE))
)
...
adex <- adex1_1 %>%
  ...
  derive_exposure_params(
    ...
    tdose, tpdose, tdurd, adose, tadj, tadjae
    ) %>%
  # add W2-24 exposure
  derive_exposure_params(
    ...
    drop_values_from = vars(EXPLDOS, EXDOSU, EXDOSFRM, EXDOSFRQ, EXROUTE, EXDURU),
    pdose, pdurd, padose, padj, padjae
  )%>%
