...
sum_overall<-exposure_var_spec(new_parameters=c("TDOSE","TDURD"),
                              input_parameters=c("DOSE", "DURD"),
                              fns= list(AVAL ~sum(., na.rm=TRUE))
)
#Average overall dose
avg_overall<-exposure_var_spec(new_parameters=c("ADOSE"),
                            input_parameters=c("DOSE"),
                            fns= list(AVAL ~mean(., na.rm=TRUE))
)
#Any Dose adjustment?
adj_overall<-exposure_var_spec(new_parameters=c("TADJ"),
                             input_parameters=c("ADJ"),
                             fns= list(AVALC ~ if_else(sum(!is.na(.)) > 0, "Y", NA_character_)))
#Any Dose adjustment due to AE?
adjae_overall<-exposure_var_spec(new_parameters=c("TADJAE"),
                            input_parameters=c("ADJAE"),
                            fns= list(AVALC ~ if_else(sum(!is.na(. == "ADVERSE EVENT")) > 0, "Y", NA_character_)))

#W2-24 expousre
sum_w2_24<-exposure_var_spec(new_parameters=c("PDOSE","PDURD"),
                               input_parameters=c("DOSE", "DURD"),
                               fns= list(AVAL ~sum(., na.rm=TRUE))
)
avg_w2_24<-exposure_var_spec(new_parameters=c("PADOSE"),
                               input_parameters=c("DOSE"),
                               fns= list(AVAL ~mean(., na.rm=TRUE))
)
adj_w2_24<-exposure_var_spec(new_parameters=c("PADJ"),
                               input_parameters=c("ADJ"),
                               fns= list(AVALC ~ if_else(sum(!is.na(.)) > 0, "Y", NA_character_)))
adjae_w2_24<-exposure_var_spec(new_parameters=c("PADJAE"),
                                 input_parameters=c("ADJAE"),
                                 fns= list(AVALC ~ if_else(sum(!is.na(. == "ADVERSE EVENT")) > 0, "Y", NA_character_)))
...
adex<-adex1_1 %>%
  ...
  derive_exposure_parameters(
    ...
    sum_overall, avg_overall,adj_overall, adjae_overall
  ) %>%
  #add W2-24 exposure
  derive_exposure_parameters(
