...
##### Where a user can quickly find some **references or advice** to use a function?
...
* Most `{admiral}` derivation functions rely on `{dplyr}` operations, which generally preserve attributes on existing columns but do not automatically transfer attributes to newly created columns.
* Admiral utility functions like `convert_blanks_to_na()` and `convert_na_to_blanks()` are explicitly designed to preserve all attributes.
...
* For detailed information about how different operations handle attributes, see the [Handling of Variable Attributes and Labels](concepts_conventions.html#attributes) section in the Programming Concepts and Conventions vignette.
