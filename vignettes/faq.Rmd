...
* Data analysis challenges in clinical trials vary depending on scientific goals, therapeutic areas, indications, data sources and data quality. We all face the same challenge with so why limit ourselves only to company-level adoption and crowd-sourcing to create ADaM datasets? 
  ...
  * Users can “slot in” user developed modules to address specific company/TA/Molecule/Study requirements
...
* People from Universitiy often bring R experience rather than a proprietary language 
...
* Complex functions potentially have an own vignette on the admiral homepage to provide a more details. 
...
* Guidance for testing and documentation expectations of community contribution functions. Then it is for each company to cover the following:
  ...
  * strategy of how the use of admiral fits into company-specific quality assurance process (e.g. Roche plan to use current SAS-based macro tool as an independent QC until   confidence builds, and then eventually could move more towards code review. GSK plan to initially use admiral as the QC method itself to compare against current SAS-based macro tool, until confidence builds)
...
* The users is responsible to make sure they use the functions and their parameters in the right way to ensure alignment with their define.xml
...
* **This is still under discussion and below is a very early draft proposal to provide an idea**
  ...
  * for core standard functions developed by the initial Roche/GSK development team, as well as community contributions to either feedback via issues or supplement with extra functions via pull requests dedicated open Github repo, where control of master branch would be kept with a select limited number of Roche/GSK technical leads 
...
* **Company-specific level - e.g. {admiral.roche} & {admiral.gsk} packages** 
   ...
   * a Github repo would be needed for each company - expectation is to also keep these open source and we can set the tone here with Roche & GSK versions. 
   * control of master branch would be kept with a select limited number of company reps for each respective company repo 
