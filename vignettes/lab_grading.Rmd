...
Using CDISC data we can map these lab tests to the correct terms, but firstly create
`PARAMCD`, `PARAM`, `AVAL`, `ANRLO`, `ANRHI` also some lab grading criteria require `BASE`
and `PCHG`, so these would also need created before running `derive_var_atoxgr_dir()`
function.
...
We can now create another look-up table to add on `ATOXDSCL` and `ATOXDSCH` using
...
grade_lookup <- tibble::tribble(
  ...
  "CHOL",    NA_character_,                 "Cholesterol high",
  ...
  "LYM",     "CD4 lymphocytes decreased",   NA_character_,
...
so comparing lab value against ULN and also BASE. The decision made was put in the
`COMMENT` field.
...
to the lab value, so take worse case and assume its a grade 3. The decision made was 
