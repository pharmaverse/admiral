...
adlb_annotated <- adlb %>%
  ...
  mutate(CRIT1 = case_when(PARAMCD == "AST" ~ "AST >=3xULN",
...
HYLAW_records <- derive_vars_joined(
    ...
    by_vars = vars(STUDYID, USUBJID, ADY),
    order = vars(ADY),
    filter_join = ADT.join - ADT <= 14 & CRIT1FL == "Y" & CRIT1FL.join == "Y",
    new_vars = vars(BILI_LBSEQ = LBSEQ, BILI_DT = ADT, BILI_CRITFL = CRIT1FL),
    mode = "first"
  )
...
HYLAW_params <- derive_param_exist_flag(
  ...
  false_value = "N",
  missing_value = "N",
  subject_keys = vars(STUDYID, USUBJID, AVISIT, ADT),
  set_values_to = vars(
    PARAMCD = "HYSLAW",
