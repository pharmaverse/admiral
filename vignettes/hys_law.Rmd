...
Using these records, you can row-bind this back into the original ADLB to create a ADHY dataset. Perhaps instead of 
```{r}
# ASTALT <- adlb3 %>% 
#   filter(PARAMCD %in% c("AST", "ALT"))
# 
# BILI <- adlb3 %>% 
#   filter(PARAMCD %in% "BILI")
# 
adlb4 <- full_join(
  filter(adlb3, PARAMCD %in% c("AST", "ALT")),
  filter(adlb3, PARAMCD %in% c("BILI")),
  by=c("USUBJID", "dummy")) %>%
  filter(POTDILI.x == "Y" & POTDILI.y == "Y") %>% 
  mutate(
    PARAMCD = "AST3BIL2",
    AVAL = if_else(abs(ADY.x - ADY.y) <= 14 ,
                  1,
                  0, 
                  missing = 0)
  ) %>%
  select(USUBJID, ADT.x, PARAMCD, AVAL) %>%
  rename(ADT = ADT.x) %>% 
  distinct()
```
