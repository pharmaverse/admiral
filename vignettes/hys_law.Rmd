...
- Elevated alanine transaminase (ALT) or aspartate transaminase (AST) by 3-fold or greater of the upper limit of normal, or 5-fold (in the case of aminotransferases).
- Elevated serum total bilirubin (BILI) by 2-fold or greater within a window of time, ~14 days of the elevated AST/ALT event.
...
Using `call_derivation()` and `derive_var_merged_exist_flag()`, we can create a variety of flags such as one to indicate the the 3-fold or greater than upper limit of normal of ALT/AST. The categorical bands and equality placement may vary with your organization. In our example, for ALT/AST we'll create 4 categories: <3xULN, >=3 - <5xULN, >=5 - <10xULN, >=10xULN.
...
By joining this dataset on itself, filtering by records that occured within a time window, for example 14 days, then selecting on distinct records of `USUBJID` and `ADT` you can identify paired records that may qualify for DILI. 
...
Alternatively, you can create a relrec-style long dataset giving each AST/ALT and BILI pair a unique key to more thoroughly conduct a manual review of potential DILI cases.
```{r}
