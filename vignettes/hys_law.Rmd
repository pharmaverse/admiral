...
adlb3 <- adlb2 %>%
  ...
  select(USUBJID, LBSEQ, PARAMCD, ADY, starts_with(c("AST", "BILI"))) %>%
  pivot_longer(.,
...
By joining this dataset on itself and filtering by records that occured within a time window, for example 14 days, you can end up with relrec shaped dataset containing all paired labs that would qualify as an event according to Hy's law.
...
dili_relrec <- inner_join(
  ...
  mutate(LBSEQ = map2(.x = LBSEQ.x,
                      .y = LBSEQ.y,
                      .f = function(x,y) sort(c(x,y)))) %>%
  group_by(USUBJID) %>%
  mutate(DILIPAIR = paste("LB-LB-PAIR", 1:n(), sep = "")) %>%
  select(USUBJID, DILIPAIR, LBSEQ) %>%
  unnest(LBSEQ)
head(dili_relrec)
```
