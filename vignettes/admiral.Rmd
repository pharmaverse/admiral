...
In this example we will simply add new records by deriving the 
`Mean Arterial Pressure` with the help of the function `derive_param_map`.

## Load Packages and Example Datasets

Similarly to the example of ADSL, we will load our packages and example datasets to
...
`ADSL` variables used for derivations are selected at this step.
...
## Derive Numeric Date and Analysis Day (`ADT`,  `ADY`)

The function `derive_vars_dt()` can be used to derive `ADT`. For date imputation, refer to the [BDS Finding ADaM](https://pharmaverse.github.io/admiral/articles/bds_finding.html) page. 
...
## Derive `MAP` Parameter

Below is finally our example of creating `Mean Arterial Pressure` for `ADVS` using
the function `derive_param_map` which is a wrapper around
[derive_param_computed()](https://pharmaverse.github.io/admiral/reference/derive_param_computed.html).
