...
start with `derive_`. The first parameter of these expects the processed
dataset. This allows to string together derivations using the `%>%` operator.
...
variable names are reserved for temporary variable within the derivation and are
removed from the output dataset. If the input dataset contains such variables an
error is issued.
