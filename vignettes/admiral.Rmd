...
dataset_vignette(
  adsl,
  display_vars = c(get_admiral_option("subject_keys"), exprs(TRTSDTM, TRTSTMF))
)
```
...
advs <- advs %>%
  derive_param_computed(
    by_vars = c(get_admiral_option("subject_keys"), exprs(AVISIT, AVISITN)),
...
```{r, eval=TRUE, echo=FALSE}
advs %>%
  arrange(!!!get_admiral_option("subject_keys"), AVISITN, PARAMCD) %>%
  dataset_vignette(
    display_vars = c(get_admiral_option("subject_keys"), exprs(AVISIT, PARAMCD, AVAL))
  )
```
...
dataset_vignette(
  adsl,
  display_vars = c(get_admiral_option("subject_keys"), exprs(FINLABDT))
)
```
