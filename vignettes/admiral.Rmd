...
To help with the examples showcased later, we will load some packages and set up some
data that we can manipulate. The `{dplyr}` and `{lubridate}` and `{stringr}` packages are
`{tidyverse}` packages and are used heavily throughout this vignette. The `{admiral}`
package also leverages the `{pharmaversesdtm}` package for example SDTM datasets which 
are from the CDISC Pilot Study.
...
[Hgher Order Functions](../reference/index.html#higher_order).
...
If you are new to expressions, consider reading our [Introduction to expressions](concepts_conventions.html#exprs) to
learn more.
