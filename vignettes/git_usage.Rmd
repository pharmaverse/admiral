...
A package release is done in two parts.  First, we declare a freeze on all admiral activity and take the code in the `main` branch on GitHub, bundle it up and send off to [CRAN](https://cran.r-project.org/).  Once CRAN approval is given and the package is available on CRAN, we then use the release button on GitHub to "release" the package onto GitHub.  This release on Github archives the version of code within the `main` branch, attaches the News/Changelog file, bundles the code into a `tar.gz` file and makes a validation report via the GitHub action `validation` from [insightsengineering/validatoR](https://github.com/insightsengineering/thevalidatoR).  Please see past [admiral releases](https://github.com/pharmaverse/admiral/releases) for reference.  
