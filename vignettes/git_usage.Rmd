...
This article will give you an overview of how the admiral project is utilizing the version-control software git and the website Github while working with RStudio.  We will go over the primary branches that house the source code for the admiral project as well as how we use **Feature** branches to address Issues.  Issues can range from bugs to enhancement that have been identified or requested by developers, users or testers.  Finally, we provide the bare minimum git commands needed to get up and running.  Please refer to the Resource section for more in-depth guidance on using git and Github.  
...
Feature Branches are where most developers will work 
...
- Click Create branch: 94_lstalvdt from `devel`
...
You can also make use of the Git Tab within RStudio to committ your changes.  A benefit of using this Tab is being able to see your changes to the file with red and green highligting.  
...
- At least one reviewer must approve the Pull Request.  Another article, [Pull Request Review Guidance](pr_review_guidance.html) provides in depth guidance on doing a proper Pull Request. 
- The reviewer must ensure that the function follows the programming strategy recommendations.  
- Any comment/question/discussion must be addressed and documented in GitHub before the Pull Request is merged and the issue closed.

Once the review is completed, and the Pull Request is merged, the branch must be removed: click “delete branch” in the Pull Request.
...
For simple merge conflicts, developers can make use of the Github interface to solve merge conflicts.  Github will show the number conflicts between the two branches.  Just like in the terminal, Github will make use of the `<<<<<<<`, `=======`, and `>>>>>>>` to highligh the conflicting sections.  
