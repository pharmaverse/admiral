...
To Derive Query Variables, we use an input dataset with a data.frame containing required columns `VAR_PREFIX`, `QUERY_NAME`, `TERM_LEVEL`, `TERM_NAME`, `TERM_ID`, and optional columns`QUERY_ID`, `QUERY_SCOPE`, `QUERY_SCOPE_NUM`.
The content of the dataset can be checked by `assert_valid_queries()`. The expected result is the input dataset with query variables derived
...
data("queries")
 adae <- tibble::tribble(
  ...
  adae <- derive_query_vars(dataset = adae , dataset_queries = queries)
