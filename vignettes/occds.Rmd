...
```{r eval=TRUE}
ae <- derive_vars_suppqual(admiral_ae, admiral_suppae)
```
...
If you wanted to additionally check an end date you could add the `end_date` argument.
Note that in this case you could set `span_period = "Y"` if you wanted occurrences that started
prior to drug intake and ongoing or ended after this time to be considered as on-treatment.
...
numeric one. Flag first occurrence of most severe adverse event:
...
For example, there can be multiple records present in `ADCM` for a single subject with the same `ASTDTM` and `CMSEQ` variables. But these records are still differ in ATC levels:
