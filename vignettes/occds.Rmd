...
record per treatment period or one record per dose is collected. However, the
drug clearance duration should be considered when matching the exposure records
with the adverse event records because the exposure dates in `EX` are usually
just the time it takes to receive the drug. For a pill it is just a point in
time and `EXSTDTC` equals `EXENDTC`. Thus the drug clearance duration should be
added to the exposure end date to get the period in which the drug is in the
body.
