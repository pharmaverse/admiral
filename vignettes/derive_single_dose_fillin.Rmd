...
```
Derive date of death and set up data cut date
to fill in missing treatment end date EXENDT 
and missing treatment end datetime EXENDTM
Alternative approach instead of death date could use last known alive date ADSL LSTALVDT
```{r}
dm_death <- dm %>%
...
# Set up datacut as character snapshot date to use in EXENDT and EXENDTM imputations
  mutate(DCUTDT = as.Date("2015-03-06"),
...
ex_mod <- ex %>%
 ...
 derive_vars_dtm_to_dt(vars(EXSTDTM)) %>%
  derive_vars_dt(
    ...
    # Maximum imputed treatment end date must not be not greater than date of death
    # or after the datacut date
