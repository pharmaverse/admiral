...
For dates, there is also the additional option to use keyword `"mid"` to
impute missing day to `15` or missing day and month to `06-30`, but note the
...
parameter which can be set to `TRUE` in cases where seconds were never collected.
...
should be set to `NULL`. This results in `NA` if the date is partial. As
...
(`TRTSDTM`) for the `min_dates` parameter.
...
can be achieved by specifying the dates for the `max_dates` parameter.
...
created calling `if_else()` and `convert_dtc_to_dtm()`. Then the time imputation
flag is created calling `compute_tmf()`. Here no date imputation flag is created
