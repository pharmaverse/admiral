...
The functions that need to do date/time imputation follow a rule that we have called **Highest Imputation**, which has a corresponding argument in all our functions called `highest_imputation`. This rule allows a user to control which components of the DTC value are imputed if they are missing. 
...
We encourage readers to explore in more detail the `highest_imputation` options in both the `_dtm()` and `_dt()` function documentations.
