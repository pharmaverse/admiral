...
This vignette is broken into three major sections.  The first section briefly explores 
the imputation rules used in `{admiral}`.  The second section focuses on imputation functions
that work on vectors.  These **vector-based** function form the backbone of `{admiral}`'s 
more powerful functions `derive_vars_dt()` and `derive_vars_dtm()`.  The final section 
moves into detailed examples that a user might face while working on ADaMs in need 
of `---DT` and `---DTM` variables. 
...
we have called **Highest Imputation**, which has a corresponding argument called
`highest_imputation` in the date/time function.  This rule allows a user to control 
which components of the DTC value are imputed if they are missing. 

The default imputation for `_dtm()` functions is "h" (hours). A user can specify that 
that no imputation is to be done by setting `highest_imputation = n`. However, for 
for `_dt()` functions the default imputation is already set as `highest_imputation = n`.

Care must be taken when deciding on level of imputation. If a component at a higher 
level than the highest imputation level is missing, `NA_character_` is returned. For example, for `highest_imputation = "D"` `"2020"` results in `NA_character_` because the month is missing.

We encourage readers to explore in more detail the `highest_imputation` options in both
the `_dtm()` and `_dt()` function documentations.

## Imputation on a Vector

In our first example, we will make use of `impute_dtc_dtm()` on `2019-10` setting
`highest_imputation = "M"`. Note the use of `date_imputation` and `time_imputation`, which
