...
`ADEX` is filtered on USUBJID == "01-701-1028" for demonstration purpose.
```{r, eval=TRUE, echo=FALSE}
adex %>%
  arrange(USUBJID,VISIT,desc(PARAMCD), EXSTDTC, EXENDTC) %>%
  dataset_vignette(display_vars = vars(USUBJID,VISIT, EXSTDTC, EXENDTC, PARAMCD, AVAL, AVALC),
                   filter = USUBJID=="01-701-1028"
                   )
```
...
`ADEX` is filtered on USUBJID == "01-701-1028" and PARAMCD %in% c("DOSE", "TDOSE") for demonstration
purpose.
```{r, eval=TRUE, echo=FALSE}
adex %>%
  ...
  dataset_vignette(display_vars = vars(USUBJID,VISIT, 
                   ...
                   filter = PARAMCD %in% c("DOSE", "TDOSE") & USUBJID =="01-701-1028"
                 )
```
...
`ADEX` is filtered on USUBJID == "01-701-1028" and PARAMCD == "DOSE", "TDOSE","TDURD","DURD", "ADJAE", "TADJAE" for demonstration purpose.
...
adex %>%
  ...
  dataset_vignette(display_vars = vars(USUBJID,VISIT,
                   ...
                   filter = PARAMCD %in% c("DOSE","ADJAE", "TDOSE","TADJAE","TDURD","DURD") 
                   & USUBJID =="01-701-1028"
                   )
```
...
`ADEX` is filtered on USUBJID == "01-701-1028" and PARAMCD == "TNDOSINT", "TDOSE", "TPDOSE" for demonstration
purpose.
...
dataset_vignette(adex, 
                 display_vars = vars(USUBJID,VISIT, EXSTDTC, EXENDTC,
                                     PARCAT1,PARAMCD,  AVAL, ASTDT, AENDT),
                 filter = PARAMCD %in% c("TNDOSINT", "TDOSE", "TPDOSE") & USUBJID =="01-701-1028"
                 )
```
