...
dataset_vignette(
  adex,
  display_vars = get_admiral_option("subject_keys"), VISIT, EXSTDTC, EXENDTC, ASTDT, AENDT)
)
```
...
dataset_vignette(
  adex,
  display_vars = get_admiral_option("subject_keys"), VISIT, EXSTDTC, EXENDTC, ASTDTM, AENDTM)
)
```
...
dataset_vignette(
  adex,
  display_vars = get_admiral_option("subject_keys"), EXDOSE, EXPLDOS, EXDURD, DOSEO, PDOSEO)
)
```
...
single_dose <- adex %>%
  ...
  create_single_dose_dataset(keep_source_vars = get_admiral_option("subject_keys"), EXDOSE, EXPLDOS, EXDOSFRQ, ASTDT, AENDT))
```

```{r, eval=TRUE, echo=FALSE}
dataset_vignette(
  single_dose,
  display_vars = get_admiral_option("subject_keys"), EXDOSE, EXPLDOS, EXDOSFRQ, ASTDT, AENDT)
)
```
...
adex %>%
  ...
  dataset_vignette(display_vars = get_admiral_option("subject_keys"), VISIT, ASTDT, AENDT, PARAMCD, AVAL, AVALC))
```
...
adex %>%
  ...
  dataset_vignette(display_vars = get_admiral_option("subject_keys"), VISIT, PARCAT1, PARAMCD, AVAL, AVALCAT1))
```
...
dataset_vignette(
  adex,
  display_vars = get_admiral_option("subject_keys"), VISIT, PARCAT1, PARAMCD, AVAL, ASTDT, ASEQ)
)
```
