...
The CDISC pilot `EX` doamin data does not contain a dose adjustment flag or 
...
This example additionally demonstrates the date imputation features available in
the `derive_vars_dtm` function:
...
It may be necessary to obtain the analysis date from an imputed analysis datetime
column.  This can by:

```{r eval=TRUE}
adex <- derive_var_astdy(adex, reference_date = TRTSDT, date = ASTDTM)
adex <- derive_var_aendy(adex, reference_date = TRTSDT, date = AENDTM)
```
...
administered, dose adjusted, etc. during the start and date record interval defined
by the exposure record. 

These records can be derived using simple `mutate` assignments and then combined:
...
A record with `PARAMCD` = "TDOSE" is created with `PARCAT` set to "OVERALL" using
the records in `ADEX` where `PARAMCD` = "DOSE" by summing `AVAL`. In addition, 
the `ASTDTM`, `AENDTM`, `ASTDT`, and `AENDT` are created as the minimum and 
maximum date/times associated with each `by_vars` grouping.

**Note**: It should be noted, if you do not have a time associated with your data, you will
need to impute your time as "00:00:00" and create the analysis datetime variable
for date functionality to work.
...
```{r eval=FALSE}
adex <- adex %>% 
  derive_param_doseint(new_param = "TDOSINT", 
                       tadm_code = "TDOSE", 
                       tpadm_code = "TPDOSE")
```
...
Again, multiple dose intensity calculations may be performed using the `call_derivation()`
function:
...
head(select(adex, USUBJID, PARAMCD, PARAM, PARAMN, AVAL))
```
