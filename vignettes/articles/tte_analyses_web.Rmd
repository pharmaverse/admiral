...
adsl <- tribble(
  ...
  "003",    ymd("2020-01-04"), ymd("2020-04-18"),
  "004",    ymd("2020-01-03"), ymd("2020-02-22")
) %>%
  mutate(
    STUDYID = "XY1234"
  )
...
```{r, echo=FALSE}
plot_scenario <- function(assessment_type, event_type) {
  if (assessment_type == "continuous") {
    dataset <- tribble(
      ~subject, ~day, ~event,  ~end_day,
      "1",        13, "Event",       38,
      "1",        32, "Event",       38,
      "1",        41, "Event",       38,
      "2",        38, "Event",       32
  ...
  dataset <- mutate(dataset, subject = paste("Subject", subject))
  dataset_tte <- mutate(dataset_tte, subject = paste("Subject", subject))
  final_plot <- ggplot(dataset) +
    ...
    scale_shape_manual(
      name = NULL,
      values = c("No event" = 1, "Unknown" = 63, "Event" = 4)
    ) +
    scale_color_manual(
      name = NULL,
      values = c("No event" = "darkgreen", "Unknown" = "blue", "Event" = "red")
    ) +
    scale_fill_manual(
      name = NULL,
      values = c("No event" = "darkgreen", "Unknown" = "blue", "Event" = "red")
    )
  if (assessment_type == "discrete") {
    final_plot <- final_plot +
      scale_x_continuous(
        breaks = seq(15, 60, by = 14),
        labels = c("Visit 1", "Visit 2", "Visit 3", "Visit 4")
      )
  }
  final_plot <- final_plot +
    scale_y_discrete(
      limits = rev(levels(as.factor(dataset$subject)))
    ) +
    guides(
    ...
    geom_segment(
      ...
      aes(
        x = day,
        xend = day,
        y = as.numeric(as.factor(rev(levels(as.factor(subject))))) + 0.3,
        yend = as.numeric(as.factor(rev(levels(as.factor(subject))))) + 0.13
      ),
      inherit.aes = FALSE,
    ...
    geom_text(
      ...
      aes(
        x = day,
        y = subject,
        label = if_else(censor == 0, "Event time", "Censoring time")
      ),
      hjust = "center",
      nudge_y = 0.35,
      inherit.aes = FALSE
...
```{r, echo=FALSE}
plot_data <- function(dataset, dataset_tte = NULL, title = NULL, subtitle = NULL) {
...
- if it did not, the last time when it is known that the subject didn't have an
event is set as the **censoring time**.
...
end of the study, death, the start of an alternative treatment, etc.
...
```{r, echo = FALSE}
plot_scenario(assessment_type = "continuous", event_type = "negative")
```
...
That is, information about the event is available only at these time points and
it may happen that for some assessments it is unknown if the event occurred or
not, e.g., tumor scans were not readable or the score couldn't be calculated
because too few questions were answered. In this case, the event time is the
time of the first assessment where the event is recorded. For the censoring time
it depends on the type of the event. If the event is a negative event like
death, worsening, adverse event, etc., the most conservative approach is to
ignore time points where it is not known if the event occurred, i.e., the
censoring time is set to the time of the last assessment where it is known that
the event didn't occur [^1].

[^1]: see for example [Clinical Trial Endpoints for the Approval of Non-Small
Cell Lung Cancer Drugs and Biologics Guidance for
Industry](https://www.fda.gov/media/116860/download)

```{r echo = FALSE}
plot_scenario(assessment_type = "discrete", event_type = "negative")
```
...
<details>
<summary>**`adqs_a` dataset** <small>(click to expand/collapse)</small></summary>
```{r, echo=FALSE}
dataset_vignette(
  adqs_a,
...
```{r, echo=FALSE}
plot_data(adqs_a, title = "Change from Baseline Categories over Time")
```
...
<details>
<summary>**`adtte` dataset** <small>(click to expand/collapse)</small></summary>
```{r, echo=FALSE}
...
plot_data(
  adqs_a,
  dataset_tte = adtte,
  title = "Event/Censoring Times for Worsening"
)
```

## Discrete Assessments, Positive Event

For positive events like improvement, response, etc., the most conservative
...
```{r, echo = FALSE}
plot_scenario(assessment_type = "discrete", event_type = "positive")
```
...
plot_data(
  adqs_a,
  dataset_tte = adtte,
  title = "Event/Censoring Times for Improvement"
)
```
...
preference. If the confirmed change is considered as a separate endpoint, CDISC
recommends to derive a separate parameter. If the confirmation is considered
only as a selection criterion for the time-to-event analysis, both options are
possible.

In the following sections three options are presented how to derive a "confirmed
worsening" parameter in `ADTTE`. The dataset and plot displayed at the end of
each section is the same but the code to derive it is different.
...
at the next visit. Only assessments within the observation period (`ANL01FL ==
"Y"`) are considered for the confirmation [^2].

[^2]: See also the [CDISC Diabetic Kidney Disease Therapeutic Area User Guide v1.0](https://www.cdisc.org/standards/therapeutic-areas/diabetic-kidney-disease/diabetic-kidney-disease-therapeutic-area-user) for a similar example.
```{r}
adqs_ext <- adqs_a %>%
...
<details open>
<summary>**`adqs_ext` dataset** <small>(click to expand/collapse)</small></summary>
```{r, echo=FALSE}
dataset_vignette(
  adqs_ext,
...
plot_data(
  adqs_ext,
  dataset_tte = adtte,
  title = "Event/Censoring Times for Confirmed Worsening"
)
```
...
`derive_extreme_event()` function can be used. The `filter_source` argument of
the `event_joined()` object is used to restrict the records to observation
period (`ANL01FL == "Y"`).
...
dataset_vignette(
  adqs_ext,
  display_vars = exprs(USUBJID, PARAMCD, AVISIT, CHGCAT1, ANL01FL, AVALC)
)
```
</details>

The new parameter `CONFCHGA` is then used in the definition of the event for
confirmed worsening.
```{r}
confirmed_worsening <- event_source(
  dataset_name = "adqs",
...
Some events are defined as a combination of multiple events. For example,
progression-free survival is defined as progression _or_ death. For events
combined by "or", you can create separate `event_source()` objects for each
event and pass them to the `event_conditions` argument of `derive_param_tte()`.
For events combined by "and" or more complex combinations, you must first
derive a variable or parameter in the input dataset that indicates whether the
combined event occurred.
...
<details>
<summary>**`adqs_all` dataset** <small>(click to expand/collapse)</small></summary>
```{r, echo=FALSE}
dataset_vignette(
  adqs_all,
...
plot_data(
  adqs_all,
  title = "Change from Baseline Categories over Time for Score A and Score B"
)
```

We define the events for improvement in score A and score B separately and then
specify both events for the `event_conditions` argument of `derive_param_tte()`.
...
plot_data(
  adqs_all,
  dataset_tte = adtte,
  title = "Event/Censoring Times for Improvement in Score A or Score B"
)
```

## Events Combined by "and"

If events are combined by "and", e.g., score A improved _and_ score B didn't
...
<details>
<summary>**`adqs_all_ext` dataset** <small>(click to expand/collapse)</small></summary>
```{r, echo=FALSE}
...
plot_data(
  adqs_all,
  dataset_tte = adtte,
  title = "Event/Censoring Times for Improvement in Score A and Stable Score B"
)
```

# Summary

The preparation of time-to-event datasets can be more or less complex depending
on the definition of the event and the collection of the data. The
`derive_param_tte()` function provides a flexible framework to derive
time-to-event parameters for a wide range of scenarios. For more guidance and
examples see the documentation of `derive_param_tte()` and the [Creating a BDS
Time-to-Event ADaM](bds_tte.html) vignette.

If your use case is not covered or you are unsure how to implement it using
admiral, please create an issue in the [admiral GitHub
repository](https://github.com/pharmaverse/admiral/issues/new/choose). Any
feedback helps us improving `{admiral}`!
