...
dataset_vignette(
  dataset = adsl_03,
  display_vars = vars(USUBJID, TRTSDT)
)
```
...
If we wanted to only flag post-baseline records here as in the nadir example,
we could use `restrict_derivation()` as follows, as explained at
[Higher Order Functions](higher_order.html).

```{r eval=TRUE}
# Highest severity flag post-baseline (AEHSPBFL)
ae_04 <- ae_03 %>%
  restrict_derivation(
...
dataset_vignette(
  dataset = ae_04,
  display_vars = vars(USUBJID, AESEV, AESTDY, AESEQ, AEHSEVFL, AEHSPBFL)
)
```
