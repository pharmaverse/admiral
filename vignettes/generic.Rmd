...
How the (new) variables are set depends on if variables, a flag, or records are
added by the derivation.
...
For common computations like BMI `{admiral}` offers [computations](../reference/#computation-functions-for-vectors). In the previous example `compute_bmi()` could be used instead of the formula for BMI:
```{r}
derive_param_computed(
  advs,
  by_vars = exprs(USUBJID, AVISIT),
