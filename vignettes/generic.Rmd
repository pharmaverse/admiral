...
will most commonly be able to be re-used to save users having to create their own
functions for study-specific analysis derivations. These re-usable functions give
...
```
```{r echo=FALSE}
dm <- filter(dm, USUBJID %in% c("01-701-1111", "01-701-1057"))
ds <- filter(ds, USUBJID %in% c("01-701-1111", "01-701-1057"))
ex <- filter(ex, USUBJID %in% c("01-701-1111", "01-701-1057"))
ae <- filter(ae, USUBJID %in% c("01-701-1111", "01-701-1057"))
```
...
Note: Only `USUBJID` and the variable(s) added are printed for illustration in
these examples.
...
Now, an example to add first treatment datetime (`ADSL.TRTSDTM`), where we now
...
```{r eval=TRUE}
# Convert exposure start date to numeric date with time imputed as earliest,
# but no date imputation applied (EXSTDT)
...
This call would return the input dataframe with variables `TRTSDTM` and `TRTSTMF`
(time imputation flag) added.

```{r, eval=TRUE, echo=FALSE}
dataset_vignette(
  dataset = adsl_03,
  display_vars = vars(USUBJID, TRTSDTM, TRTSTMF)
)
```
...
dataset_vignette(
  dataset = adsl_04,
  display_vars = vars(USUBJID, SAFFL)
)
```
...
`derive_var_merged_cat()` to create a categorization variable from the information
joined from the additional dataframe.
...
wanted the date populated for patients in the safety population. You would now need
to check both the additional `DS` and the original `ADSL` dataframes, so this would
not be possible in one step using the "merged" functions.

**Examples**\

The above mentioned randomization date for safety population patients variable
(let's call it `SAFRANDT` here) would use `derive_vars_joined()` as follows.
...
dataset_vignette(
  dataset = adsl_05,
  display_vars = vars(USUBJID, RANDDT, SAFRANDT)
)
```
...
dataset_vignette(
  dataset = ae_01,
  display_vars = vars(USUBJID, AEDECOD, AESTDY, DCUTFL)
)
```
...
dataset_vignette(
  dataset = ae_02,
  display_vars = vars(USUBJID, AESEV, AESTDY, AENADSEV)
)
```
...
set for your observation if both the obeservation and some other observation meet both
...
Building on the above example, let's use `derive_var_extreme_flag()` to now flag
...
dataset_vignette(
  dataset = ae_03,
  display_vars = vars(USUBJID, AESEV, AESTDY, AESEQ, AEHSEVFL)
)
```
