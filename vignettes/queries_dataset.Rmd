...
Because the definition, collection and storage of the medical concepts is
company specific, the dataset passed to the `dataset_queries` argument of the
`derive_vars_query()` function should be created outside of admiral in a
specific format so that it can be passed in the function. The admiral function
`create_query_data()` can be used to create queries datasets.
