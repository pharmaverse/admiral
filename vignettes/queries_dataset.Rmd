...
To support the analysis of the safety analysis, it is quite common to define specific grouping of events. One of the most common way is to group events or medications by a specific medical concept such as a Standard MedDRA Queries or WHO-Drug Standardized Drug Grouping . 


To help with the derivation of these variables, the admiral function `derive_vars_query()` can be used.
This function takes as input the dataset (`dataset`) where the grouping must occurs (e.g `ADAE`) and
...
### Output
  ...
  + **Result:**
    ...
    Note: The type (numeric or character) of the variable defined in `TERM_LEVEL` is checked in `dataset`. 
    
    **IS THAT CORRECT ??** If the type of the variable is character then `TERM_NAME` will be used to populate `ABCzzNAM`, if it is numeric, `TERM_ID` will be used.


### Example

In this example, 1 standard MedDRA query (`VAR_PREFIX = "SMQ01"`) and 1 customized query (`VAR_PREFIX = "CQ02"`) are  defined to analyze the adverse events. 
  ...
  + The customized query (`CQ02NAM`) [`VAR_PREFIX`] will be populated with "Query 2" [`QUERY_NAME`] if any Low Level Term Code (`AELLTCD`)  [`TERM_LEVEL`] in `dataset` is equal to 10  [`TERM_ID`].
  
  **`dataset_queries = ds_query`: **
...
0001 | AE3 | 10| | 
  ...
  **output dataset from `derive_vars_query(dataset = ae, dataset_queries = ds_query)`:**
...
0001 | AE3 | 10| | Query 2
  ...
  Subject 0001 has 1 event meeting the Standard Query 1 criteria (`AEDECOD = "AE1"`) and 1 event meeting the customized query (`AELLTCD = 10)`. 
...
VAR_PREFIX | QUERY_NAME | QUERY_ID | QUERY_SCOPE |QUERY_SCOPE | **Variables created**
------- | ----- | ------ | ----- | ----- | -----
