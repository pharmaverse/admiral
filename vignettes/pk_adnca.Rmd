...
dataset_vignette(
  pc_dates,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(PCTEST, ADTM, VISIT, PCTPT, NFRLT
                   )
  )
)
```
...
dataset_vignette(
  ex_dates,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(EXTRT, EXDOSFRQ, ASTDTM, AENDTM,
                         VISIT,VISITDY, NFRLT)
  )
)
```
...
dataset_vignette(
  ex_exp,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(DRUG,EXDOSFRQ, ASTDTM, AENDTM,
                         AVISIT, NFRLT)
  )
)
```
...
dataset_vignette(
  adpc_first_dose,
  display_vars = c(
   get_admiral_option("subject_keys"), exprs(FANLDTM, AVISIT, ADTM, PCTPT
    )
  )
)
```
...
dataset_vignette(
  adpc_prev,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(VISIT, ADTM, VISIT, PCTPT,
                         ADTM_prev, EXDOSE_prev, AVISIT_prev
                   )
  )
)
```
...
dataset_vignette(
  adpc_next,
  display_vars = c(get_admiral_option("subject_keys"),
    exprs(VISIT, ADTM, VISIT, PCTPT, ADTM_next, EXDOSE_next,
          AVISIT_next)
  )
)
```
...
dataset_vignette(
  adpc_nom_next,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(NFRLT, PCTPT, NFRLT_prev, NFRLT_next
                   )
  )
)
```
...
dataset_vignette(
  adpc_arrlt,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(FANLDTM, AVISIT, PCTPT, AFRLT,
                         ARRLT, AXRLT)
  )
)
```
...
dataset_vignette(
  adpc_nrrlt,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(AVISIT, PCTPT, NFRLT, NRRLT, NXRLT
                   )
  )
)
```
...
dataset_vignette(
  adpc_aval,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(NFRLT, AVISIT, ATPT, ABLFL,
                         ATPTREF, AVAL, AVALCAT1)
  )
)
```
...
dataset_vignette(
  dtype,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(DTYPE, ATPT, NFRLT, NRRLT,
                         AFRLT, ARRLT, BASETYPE)
  )
)
```
...
```{r, eval=TRUE, echo=FALSE}
adpc_dtype %>%
  dataset_vignette(display_vars = c(get_admiral_option("subject_keys"), exprs(BASETYPE, ADTM,
                                    ATPT, NFRLT, NRRLT,
                                    ARRLT, MRRLT)
    )
  )
...
```{r, eval=TRUE, echo=FALSE}
adpc_aseq %>%
  dataset_vignette(display_vars = c(get_admiral_option("subject_keys"), exprs(BASETYPE, DTYPE, AVISIT, ATPT, AVAL, NFRLT, NRRLT, AFRLT, ARRLT, BASE, CHG
    )
  )
)
```
...
```{r, eval=TRUE, echo=FALSE}
adpc_baselines %>%
  dataset_vignette(display_vars = c(get_admiral_option("subject_keys"), exprs(HTBL, HTBLU, WTBL, WTBLU, BMIBL, BMIBLU, BASETYPE, ATPT, AVAL)
  )
)
```
...
dataset_vignette(
  adppk_first_dose,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(FANLDTM, AVISIT, ADTM, PCTPT
    )
  )
)
```
...
dataset_vignette(
  adppk_aprlt,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(EVID, NFRLT, AFRLT, APRLT, NPRLT
    )
  )
)
```
...
dataset_vignette(
  adppk_aval,
  display_vars = c(get_admiral_option("subject_keys"),
                   exprs(EVID, DOSEA, AMT, NFRLT, AFRLT,
                         CMT, DV, MDV, BLQFN)
  )
)
```
...
dataset_vignette(
  adppk_aseq,
  display_vars = c(
    get_admiral_option("subject_keys"), exprs(EVID, DOSEA, AMT, NFRLT, AFRLT, CMT, DV, MDV, BLQFN, ASEQ
  ))
)
```
