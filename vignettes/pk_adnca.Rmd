---
title: "Creating a PK NCA ADaM"
output: 
  rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Creating a PK NCA ADaM}
...
Non-compartmental analysis (NCA) ADaM using CDISC Implementation Guide
(<https://www.cdisc.org/standards/foundational/adam/adamig-non-compartmental-analysis-input-data-v1-0>).
Examples are currently presented using an underlying `EX` and `PC`
domains where the `EX` and `PC` domains represent data as collected and
the `ADPC` ADaM is output. However, the example can be applied to
situations where an `EC` domain is used as input instead of `EX` and/or
`ADNCA` or another ADaM is created.
...
dose. Pre-dose records may refer to the upcoming dose. The CDISC
Implementation guide makes use of duplicated records for analysis, which
allows the same record to be used both with respect to the previous dose
and the next upcoming dose. This illustrated later in this vignette.

These are the relative time variables we will use. These correspond to
...
-   [Find First Dose and Calculate `AFRLT`](#firstdose)

-   [Find Dose Dates Corresponding to PK Records](#dosedates)

-   [Combine `PC` and `EX` Records and Derive Relative Time
    Variables](#relative)

-   [Derive Analysis Variables `AVAL`, `ATPT`, `ATPTREF` and
    `BASETYPE`](#analysis)
...
baseline variables needed. These may come from either the SDTM or ADaM
source.
...
```{r echo=FALSE}
# ex <- filter(ex, USUBJID %in% c("01-701-1028", "01-701-1033", "01-701-1442", "01-714-1288"))
# pc <- filter(pc, USUBJID %in% c("01-701-1028", "01-701-1033", "01-701-1442", "01-714-1288"))
```
...
selected at this step. The rest of the relevant `ADSL` variables would
be added later.
...
`derive_vars_dtm_to_tm`, `derive_vars_dy`. We will also create `NFRLT`
for `PC` data based on `PCTPTNUM`. We will create an event ID (`EVID`)
of 0 for concentration records and 1 for dosing records. This is a
...
dataset_vignette(
  ex_exp,
  display_vars = vars(USUBJID, DRUG, EXDOSFRQ, ASTDTM, AENDTM, AVISIT, NFRLT)
)
```
...
We also create and analysis visit (`AVISIT`) based on `NFRLT`. The first
dose datetime for an analyte `FANLDTM` is the calculated as the minimum
...
dataset_vignette(
  adpc,
  display_vars = vars(USUBJID, FANLDTM, AVISIT, ADTM, PCTPT)
)
```
...
Use the same method to find and previous and next nominal times. Note
...
supposed to be. Sometimes this will differ from the actual times in a
...
dataset_vignette(
  adpc,
  display_vars = vars(USUBJID, FANLDTM, AVISIT, PCTPT, AFRLT, ARRLT, AXRLT)
)
```
...
dataset_vignette(
  adpc,
  display_vars = vars(USUBJID, NFRLT, AVISIT, PCTPT, NRRLT, NXRLT)
)
```

## Derive Analysis Variables `AVAL`, `ATPT`, `ATPTREF` and `BASETYPE` {#analysis}
...
dataset_vignette(
  adpc,
  display_vars = vars(USUBJID, NFRLT, AVISIT, ATPT, ABLFL, ATPTREF, AVAL, AVALCAT1)
)
```
...
dataset_vignette(
  dtype,
  display_vars = vars(USUBJID, DTYPE, ATPT, NFRLT, NRRLT, AFRLT, ARRLT, BASETYPE)
)
```
...
```{r, eval=TRUE, echo=FALSE}
adpc %>%
  dataset_vignette(display_vars = vars(STUDYID, USUBJID, BASETYPE, ADTM, ATPT, NFRLT, NRRLT, ARRLT, MRRLT))
```
...
example](https://examples.pharmaverse.org/data/adsl/).
