...
Non-compartmental analysis (NCA) ADaM using the CDISC Implementation
Guide
...
datasets---which are included in `{admiral.test}`---are used.
...
dataset_vignette(
  adpc,
  display_vars = vars(USUBJID, AVISIT, PCTPT, NFRLT, NRRLT, NXRLT)
)
```
