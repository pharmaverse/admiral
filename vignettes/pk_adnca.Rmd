...
dataset_vignette(
  ...
  display_vars = c(
    get_admiral_option("subject_keys"),
    exprs(
      VISIT, ADTM, VISIT, PCTPT, ADTM_next, EXDOSE_next,
      AVISIT_next
    )
  )
)
...
adpc_arrlt <- bind_rows(adpc_nom_next, ex_exp) %>%
  group_by(!!!get_admiral_option("subject_keys"), DRUG) %>%
  mutate(
    FANLDTM = min(FANLDTM, na.rm = TRUE),
    min_NFRLT = min(NFRLT_prev, na.rm = TRUE),
    maxdate = max(ADT[EVID == 0], na.rm = TRUE), .after = USUBJID
  ) %>%
  arrange(!!!get_admiral_option("subject_keys"), ADTM) %>%
  ungroup() %>%
  filter(ADT <= maxdate) %>%
...
```{r, eval=TRUE, echo=FALSE}
adpc_dtype %>%
  dataset_vignette(display_vars = c(get_admiral_option("subject_keys"), exprs(
    BASETYPE, ADTM,
    ATPT, NFRLT, NRRLT,
    ARRLT, MRRLT
  )))
