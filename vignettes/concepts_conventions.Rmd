...
Computers store numeric values as floating point numbers, which cannot represent
most decimal numbers exactly. Therefore, the result of a calculation may differ
slightly from the mathematically correct value, even if the printed value looks
correct.
...
This is important to keep in mind when a condition compares a *derived* numeric
variable (e.g., `CHG`, `PCHG`, ratios, or unit conversions) to a fixed value.
Values which are mathematically exactly on the boundary may be flagged
incorrectly. Many `{admiral}` functions accept conditions, e.g.,
`derive_vars_crit_flag()`, `derive_vars_cat()`, or any function with a
`filter_add` argument, and are thus affected.
...
- Round the derived variable to a reasonable number of significant digits
before comparing it, e.g., by using `signif()`. `{admiral}` uses this approach
internally, e.g., in `derive_var_anrind()`, where the number of significant
digits can be specified by the `signif_digits` admiral option (see
`set_admiral_options()`).
...
- Alternatively, the derived variable can be rounded when it is derived, e.g.,
`mutate(PCHG = round(PCHG, 5))`, if this is appropriate for the analysis.

- Use a tolerance-based comparison with `dplyr::near()`.
