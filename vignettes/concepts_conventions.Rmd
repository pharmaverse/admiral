...
# Argument Conventions (to be moved to other page)
...
`expr()` is a function from the `{rlang}` package, which is used to an **expression**. The expression is not evaluated 
...
# This doesn't work as we are not unquoting the subj ect keys
...
In summary, although the expressions framework may seem slightly clunky and un-intuitive to begin with, it allows for such power
and flexibility that it forms a key part of the `{admiral}` package. For a comprehensive treatment of expressions, see 
[Chapter 18](https://adv-r.hadley.nz/expressions.html) and [Chapter 19](https://adv-r.hadley.nz/quasiquotation.html) of the Advanced R textbook. Chapter 19 specfically covers in much more detail the concept of unquoting.
...
The second call fails because hidden within the expression is a mention of `VSTPT`, which is a variable missing from `vs`.
