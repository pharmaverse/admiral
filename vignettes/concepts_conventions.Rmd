...
`{admiral}` family of packages. This includes how missing values are handled in `{admiral}`, as well as the way in which users are expected to pass inputs to `{admiral}` functions, and how those inputs should look.
...
As Uncle Ben from Spiderman would say, "With great power comes great responsibility", and this couldn't ring truer when working 
with expressions: their power and flexibility can lead to misunderstandings about their functionality Let's set up some dummy data
to explore some common issues that new (or experienced!) programmers may encounter when dealing with expressions.
...
When writing more com `{admiral}` code it can be easy to mistakenly pass the wrong input to an argument that 
...
To fix this code, we need to unquote `my_expression` so that the expression that it is holding is passsed 
correctly to `derive_vars_merged()`:
...
sure that it can be evaluated within the dataset of interest. For intance, consider this code:
