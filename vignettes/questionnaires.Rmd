...
(GDSSF)](https://www.cdisc.org/standards/foundational/qrs/geriatric-depression-scale-short-form-0)):
```{r echo=FALSE}
...
qs_gdssf <- tibble::tribble(
  ...
  46L, "GDS02- Total Score - Analysis", "GDS02TS", "UNSCHEDULED 2.01", 201, "2012-12-28", NA, 9L,
...
adsl <- tibble::tribble(
  ~STUDYID, ~USUBJID, ~SITEID, ~ITTFL,    ~TRTP, ~TRTSDT,
  "STUDYX",  "P0001",     13L,    "Y", "DRUG A", lubridate::ymd("2012-11-16")
)
```
# Original Items
...
dataset_vignette(
  adqs,
  display_vars = exprs(USUBJID, PARAMCD, PARAM, PARCAT1, AVALC, AVAL, AVISIT, AVISITN))
```
...
dataset_vignette(
  adgad7,
  display_vars = exprs(USUBJID, PARAMCD, PARAM, AVAL, AVISIT)
)
```

For the GDSSF questionnaire the total score is defined as the average of the item values transformed to the range [0, 15]. If more than five items are missing the total score is considered as missing. This parameter can be derived by
