...
```{r eval=FALSE}
advs_adsl <- left_join(vs_t, adsl, by = c("STUDYID", "USUBJID"))
```
...
`advs_dt <- derive_vars_dt(advs_adsl, new_vars_previx = "A", dtc = VSDTC)`
...
```{r eval=FALSE}
advs_paramcd <-  left_join(advs_aval, param_lookup, lookup, by = "VSTESTCD")
````
...
```{r eval=FALSE}
ontrtfl <- derive_var_ontrtfl(advs_anl01fl, 
                              date = ADT, 
...
```{r eval=FALSE}
advs_ontrtfl <- derive_var_ontrtfl(
  advs_anl01fl,
  date = ADT,
  ref_start_date = TRTSDT,
  ref_end_date = TRTEDT,
...
`advs_anrind <- derive_var_anrind(advs_ontrtfl)`
...
```{r eval=FALSE}
advs_ablflablfl <- derive_extreme_flag(
  advs_basetype,
  new_var = ABLFL,
