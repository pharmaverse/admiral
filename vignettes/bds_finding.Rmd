...
The `suppvs` domain can be joined to the `vs` domain using the function `derive_suppqual_vars()`.
...
```{r eval=FALSE}
vs <- derive_suppqual_vars(vs, suppvs)
```
...
Optionally derive new parameters creating PARAMCD, PARAM, AVAL (and any other
variables). Below is an example of creating MAP for ADVS, see also Example 3 in
section below [Derive New Rows](#additional) for alternative way of creating
new PARAMCDs
...
Likewise, function call below, to create ADVS PARAMCD of 'BSA',
see also Example 3 in section below ### Derive New Rows {#additional} 
for alternative way of creating new PARAMCDs
...
head(select(filter(advs, PARAMCD %in% c("BSA", "MAP") & USUBJID == "01-701-1015"
                   & VISIT == "SCREENING 1"),
            PARAMCD, PARAM, PARAMN, PARCAT1, PARCAT1N))
```

Similarly, if running ADEG, the PARAMCDs of 'QTCBF' 'QTCBS' and 'QTCL' can be
created with function call. See example below for PARAMCD = `QTCF'.
