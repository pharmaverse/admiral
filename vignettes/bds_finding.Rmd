...
The `check_not_mapped_type` argument is used to control whether a message, 
warning, error, or no message at all is issued when some records are not mapped.
This can be useful as a failsafe to flag that some records are not being mapped 
using the lookup table, which can often occur in cases where new data comes 
through and the lookup table has not been updated.
...
from the look-up table (`PARAM`, `PARCAT1`,...) should be added.
