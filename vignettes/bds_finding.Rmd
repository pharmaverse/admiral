...
At this stage, only `PARAMCD` required to perform the derivations. Additional
...
observation before end of treatment. Set `AVISIT` to `End of Treatment` and assign
a unique `AVISITN` value.
...
example of creating `Mean Arterial Pressure` (`PARAMCD`=`MAP`) with an 
alternative formula.
