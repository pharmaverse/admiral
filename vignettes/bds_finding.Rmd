...
dataset_vignette(
  advs,
  display_vars = get_admiral_option("subject_keys"), VSTESTCD, VSDTC, VISIT, TRTSDT, TRTEDT, TRT01A, TRT01P),
...
dataset_vignette(
  advs,
  display_vars = get_admiral_option("subject_keys"), VISIT, VSDTC, ADT),
...
dataset_vignette(
  advs,
  display_vars = get_admiral_option("subject_keys"), VISIT, VSDTC, ADT, ADTF),
...
dataset_vignette(
  advs,
  display_vars = get_admiral_option("subject_keys"), VISIT, ADT, ADY, TRTSDT),
  filter = USUBJID == "01-716-1024"
)
```
...
dataset_vignette(
  arrange(advs, USUBJID, VISITNUM, VSTPTNUM, ADT, PARAMCD),
  display_vars = get_admiral_option("subject_keys"), VSTESTCD, PARAMCD, VISIT, VSTPT, AVAL),
...
adeg <- derive_param_qtc(
  adeg,
  by_vars = get_admiral_option("subject_keys"), VISIT),
  method = "Fridericia",
...
dataset_vignette(
  advs,
  display_vars = get_admiral_option("subject_keys"), BASETYPE, PARAMCD, ADT, TRTSDT, ATPTN, TRTSDT, ABLFL),
...
dataset_vignette(
  advs,
  display_vars = get_admiral_option("subject_keys"), BASETYPE, PARAMCD, ABLFL, BASE, ANRIND, BNRIND),
  filter = PARAMCD == "DIABP" & VISIT %in% c("WEEK 2", "BASELINE")
)
```
...
dataset_vignette(
  advs,
  display_vars = get_admiral_option("subject_keys"), VISIT, BASE, AVAL, ANRLO, R2BASE, R01ANRLO),
  filter = PARAMCD == "DIABP" & VISIT %in% c("WEEK 2", "WEEK 8")
)
```
...
advs <- restrict_derivation(
  ...
  derivation = derive_var_extreme_flag,
  args = params(
    by_vars = get_admiral_option("subject_keys"), BASETYPE, PARAMCD, AVISIT),
...
dataset_vignette(
  advs,
  display_vars = get_admiral_option("subject_keys"), PARAMCD, AVISIT, AVAL, ADT, ATPTN, WORSTFL),
...
advs_ex2 <- derive_summary_records(
  advs,
  dataset_add = advs,
  by_vars = get_admiral_option("subject_keys"), PARAMCD, VISITNUM, ADT),
  set_values_to = exprs(
...
dataset_vignette(
  ...
  display_vars = get_admiral_option("subject_keys"), PARAMCD, VISITNUM, ADT, AVAL, DTYPE),
  filter = USUBJID == "01-701-1015"
)
```
...
advs_ex3 <- derive_param_computed(
  advs,
  by_vars = get_admiral_option("subject_keys"), VISIT, ATPT),
...
dataset_vignette(
  ...
  display_vars = get_admiral_option("subject_keys"), PARAMCD, VISIT, ATPT, AVAL),
