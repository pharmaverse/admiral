...
Note: In `ADAB` dataset, `BASETYPE` represents Phase or Period but the method for assigning `BASE` variable
could be appended to `BASETYPE` (i.e. "DOUBLE_BLINDED LAST"). 
`BASETYPE` is carried through the remaining steps of this template as a BY 
variable for computing parameters. For example, if there is a main
phase and a later maintenance phase, you might want to compute all the ADA 
parameters by `BASETYPE`. If re-baselining does not occur in a later phase, the 
