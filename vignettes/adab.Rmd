...
NOTE: This is document is in draft state, still in the works by author 
and/or user feedbacks.
...
assembling he final dataset into original, by visit and parameterized records 
...
an model or implementatin buide for ADA (ADAMB). There is a committee and the
elements of this template reflect methods from a couple members and can serve
as a baseline for when a guide is released.  CDISC ADaM conventions and similiar
domains (such as ADPC) are followed.


# Section 1 Staging the data ---------------------------------------------------
...
domain is intended die multiple classes of findings so a dedicated 'IS_ADA' was
added to the pharmaversesdtm for ADA data.

One of the important aspects of the dataset is the separate derivation of
original 'by visit' data, by visit inteprested results and parameter variables
(like ADEX summary parameters).  A good analolgy would be ADEX that has a mix
...
# `ADAB` Programming Workflow

-   [Read in Data](#readdata)

-   [Find First Dose Date using 'EX' ](#firstdose)

-   [Merge `IS` and `EX` Records and Derive Relative Time
    Variables](#relative)

-   [Derive Asssigned Variables](#assigned)

-   [Calculate Change from Baseline and Assign `ASEQ`](#aseq)

-   [Derive Analysis Variables](#analysis)

-   [Populate `DTYPE` for Imputed values](#lloq)

-   [Add Additional Population Flag Variables](#baselines)
...
# Similiar to ADEX, assign a value for parameterized summary records
...
This code provide a mechanic so process either version, see below comments for usage.
...
DRUG (similiar to ADPC.DRUG) is an assigned variable so IS Analytes can be merged matching 
...
date part variables usign  `derive_vars_dtm_to_dt()`
...
Note: In ADAB, BASETYPE is Phase or Period but the method for assiging BASE
...
At this step, we assign MRT (Mininum Reportable Titer) and an Optional
DTL (both are user/protocol specfic assigned values).  RESULTC is the
...
values supplied to demonstrate.). RESULTN is assigned 0/1 as the numeric version fo RESULTC.
...
could be considred negative while non missing post baseline might required
(or considered Negative if sponsor defined as such).  This examplwe allows
...
# Get base only data for use later ina  FULL JOIN.
...
This section computes main ADA analye parameters into a horizontal dataset (to later be PARMAMs)
Sections are commented appropriately. 
Final Key Variables this section:
Subject ByVisit analysis variables (sponsor or molecule specific or optional)
TFLAGV - By visit Treatment Emergent Category (0=,1=,2=,3=).
PBFLAGV - Post Baseline Status y Visit
...
TFLAG  - Overall Treatment Emergent Category
...
parent main analyte.  Reminderthat ADA and Nab are drug specific.  Drug ABC can have ADA
...
ADATAT="Positive" on the main parent matching analyte. If not ADASTAT  = Positive,  NABSTAT is 
...
LPPDT  - Date of Last ADA Postive
ADADUR - Duration of ADA (LPPDT - FPPDT)
TIMADA - Time to ADA Postive (from first dose)
INDUCED - Negative at Baseline to Positve
ENHANCED - Positive as Baseline to Positive where CHG (ISSTRESN) > 0.60
...
ADABLPFL - Baseline ADA Evaluable - Baseline test exist (ABLFL = Y or sponsor defined)
...
# Section 3 - Prepare and create the final ADAB PARAM based data set ------------

In this section, we turn original IS records, the duplicated interpreted records plus
each Analysis variable into separate preparaed data frames.  These will then be
...
The adjustments are dependent on whether IS is basedon SDTM v2.0 or prior to v1.8.
