...
If your `IS` dataset has values in `ISTESTCD` and `ISTEST` different then the example (or has 
additional tests suitable for the `ADAB` domain), adjust this step accordingly. 

`ADAPARM` and `ADATYPE` are assigned to assure consistent merge variables as values are calculated
...
a common method is to use the time part from `EX.EXSTDTC` (matched by the `IS` and `EX` date parts), 
then the `IS` time is adjusted for pre-dose sampling planned time. User would add custom code for this.
...
the 'IS' analytes. As with 'IS' dataset, `DRUG` is assigned as a common merge variable.
As with 'IS' dataset, `NFRLT` is assigned using `VISITDY` (or the user could substitute
`EX.VISIT` values).

Assign `ASTDTM` using `EX.EXSTDTC`. As previously seen with `IS.ISDTC`, if `EX.EXSTDTC` is missing the time part,
"00:00" is used for the imputation. Individual companies and studies may have custom imputation methods.
If this is the case, the function `derive_vars_dtm()` would be either replaced or post-processed to 
get the desired imputed time part. For example a common method is to use the time part from `IS.ISDTC` 
(matched by the `IS` and `EX` date parts), then the `EX` missing time is adjusted for 
...
# Add study specific code as applicable using ADEX or ADSL APx and PHx variables
...
the standard interpreted result (`POSITIVE` or `NEGATIVE`) based values found
...
# Use the "post_data" dataset to create the ADPBLPFL flag, which will be merged back later in the program
...
`ADATAT="Positive"` on the main parent matching analyte. If not `ADASTAT  = "Positive"`, `NABSTAT`
...
then we compute the remaining analysis variables into a final horizontal dataset 
...
`PERSADA` -  Persistent ADA (either of):
             ...
             `INDUCED` and multiple post-baseline positive (other than last result)
                and duration between first and last positive result is >= 16 weeks.
`TRANADAE`  - Any Treatment Emergent Transient ADA (either of): 
             `INDUCED` and Positive at any Post-Baseline other than last result
             `INDUCED` and multiple post-baseline positive (other than last result)
                and duration between first and last positive result is < 16 weeks.
`PERSADAE` - Any Treatment Emergent Persistent ADA (either of):
             ...
             `INDUCED` or `ENHANCED` and multiple post-baseline positive (other than last result)
...
# data set (is_flagdata from is_visit_flags)
...
# Drop temporary variables and ADA Flag variables that are now parameterized, further below is a final 'select' statement to
