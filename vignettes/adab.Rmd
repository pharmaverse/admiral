...
`NFRLT` is assigned to the `EX` data from `EX.VISITDY` using `derive_var_nfrlt()`.
...
a method to assign `AVAL` using the assigned MRT value, sets `AVALC` to "N.C." (Not Calculable)
and documents the operation by setting `DTYPE = "MRT"`.  
...
is_baseline <- is_aval %>%
  # Calculate ABLFL. Select unique records. If there are more than one record for the `order` and `filter` it will throw a duplicate
