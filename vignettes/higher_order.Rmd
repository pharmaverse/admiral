...
vs_without <- vs %>%
  derive_var_extreme_flag(
    by_vars = c(get_admiral_option("subject_keys"), exprs(VSTESTCD)),
...
vs_without %>%
  arrange(!!!get_admiral_option("subject_keys"), VSTESTCD, VSDY, VSSEQ) %>%
  dataset_vignette(
    display_vars = c(get_admiral_option("subject_keys"), exprs(VSTESTCD, VSORRES, ALOFL, AHIFL)),
...
```{r, eval=TRUE, echo=FALSE}
ae %>%
  arrange(!!!get_admiral_option("subject_keys"), AESTDY, AESEQ, desc(TEMP_AESEVN)) %>%
  dataset_vignette(
    display_vars = c(get_admiral_option("subject_keys"), exprs(AEDECOD, AESTDY, AESEQ, AESEV, AHSEVFL))
  )
```
...
```{r, eval=TRUE, echo=FALSE}
ae %>%
  arrange(!!!get_admiral_option("subject_keys"), AESTDY, AESEQ) %>%
  dataset_vignette(
    display_vars = c(get_admiral_option("subject_keys"), exprs(AEDECOD, AESTDY, AESEQ, AESEV, AHSEV3FL))
  )
```
