...
ae <- ae %>%
  filter(USUBJID %in% c("01-701-1111", "01-705-1393")) %>%
  # keep the first AE per subject and date so the event date is a unique key for
  # the time-to-event example
  filter_extreme(by_vars = exprs(USUBJID, AESTDTC), order = exprs(AESEQ), mode = "first")
vs <- vs %>%
  filter(
