...
+ the main title (??? or another section???) level for a function, 
...
deprecate_warn("x.y.z", "fun_xxxxx_x()", "new_fun_xxxxx_x()")

# Error if the deprecated function does not exist anymore
rlang::abort(paste(
    err_msg <- paste(
      "The function `fun_xxxxx_x()` is deprecated",
      "as of admiral x.y.z.",
      "Please use the new function `new_fun_xxxxx_x()` instead."
    ))
### END DEPRECIATION
```
...
### BEGIN DEPRECIATION
  if (!missing(old_param)) {
    rlang::abort(paste(
    err_msg <- paste(
      "The `old_param` argument of `fun_xxx()` is deprecated",
      "as of admiral x.y.z.",
      "Please use the `new_param` arguments instead."
    ))
    )
    abort(err_msg)
  }
