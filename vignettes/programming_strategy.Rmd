...
variables of the input dataset are accidentically overwritten. The temporary
...
If a parameter accepts one or more variables as input, the variables should be wrapped inside `exprs()`.
...
```
    ...
    new_var = exprs(mydtm = convert_dtc_to_dtm(impute_dtc(cmstdtc,
...
In addition to the roxygen2 parameters, `@keywords` is also used. 

The keywords are used to categorize the function. Please see section "Categorization of functions".
...
or the pipe operator `magrittr::%>%`. To export these functions, the following R
...
If the parameter is renamed or replaced, a **warning** must be issued and the
new parameter takes the value of the old parameter until the next release. Note:
parameters which are not passed as `exprs()` argument (e.g. `new_var = VAR1` or
...
When writing the unit test, check that the error or warning has the right class, i.e. "lifecycle_error_deprecated" or "lifecycle_warning_deprecated", respectively. The unit-test should follow the corresponding format, per the [unit test guidance](unit_test_guidance.html#writing-unit-tests-in-admiral-):
...
the following function calls flag the special Adverse Events or a comment that highlights a molecule specific endpoint
