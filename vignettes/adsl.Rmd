...
For example purpose, the CDISC Pilot SDTM datasets---which are included in `{admiraltest}`---are used.
...
```{r eval=TRUE}
adsl <- adsl %>%
  derive_var_dthcaus(src_ae, src_ds, source_datasets = list(admiral_ae = admiral_ae, admiral_ds = admiral_ds))
```
...
adsl <- adsl %>%
  derive_var_extreme_dt(
    ...
    source_datasets = list(admiral_ae = admiral_ae, adsl = adsl, admiral_lb = admiral_lb),
    mode = "last"
  )
```
