...
* [Derive Randomization Date (`RANDDT`)](#randomization_date)
...
By default, the function will map DS records where DSDECOD="RANDOMIZED". To derive 
the Randomization Date (`RANDDT`), the call would be:
...
This call would return the input dataset with the column `RANDDT` and by default
the associated date imputation flag (`RANDDTF`) are added. If the imputation flag
is not required, the user must set the argument `flag_imputation` to FALSE.

```{r, eval=TRUE, echo=FALSE}
dataset_vignette(adsl, display_vars = vars(USUBJID, ARM, ACTARM, RANDDT))
```
