...
* [Derive Birth Date and Analysis Age (`BRTHDT`, `AAGE`, `AAGEU`)](#aage)
...
required to convert the variable `EXSTDTC` and `EXENDTC` to datetime variables
...
The function `derive_vars_aage()` can be used to derive analysis age (`AAGE`) and 
analysis age unit (`AAGEU`). The function derives the age based on the birth date 
(`BRTHDT`) and a reference date, which is typically the randomization date (`RANDDT`).
...
Typically, dates of birth are collected only as years. However, this data has complete birth dates. The function `derive_vars_dt()` has the flexibility to do imputation on partial dates. Please see the `derive_vars_dt()` documentation and examples for more details or check out the [Date and Time Imputation](imputation.html) vignette.
...
the user to impute the date as well. If you have partial dates and are in need of imputation 
then please see the `derive_vars_dt()` documentation and examples for more details 
or check out the [Date and Time Imputation](imputation.html) vignette. 
...
The following functions are helpful for many ADSL derivations:
  ...
  - `derive_vars_merged_summary()` - Merge Summary Variables
...
---- | --------------
[`ADSL`](templates.html#adsl) | `use_ad_template("ADSL")`
