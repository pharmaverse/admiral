...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), RFSTDTC, COUNTRY, AGE, SEX, RACE, ETHNIC, ARM, ACTARM)
)
```
...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), RFSTDTC, TRTSDTM, TRTSDT, TRTEDTM, TRTEDT, TRTDURD)
)
```
...
dataset_vignette(
  ds_ext,
  display_vars = get_admiral_option("subject_keys"), DSCAT, DSDECOD, DSTERM, DSSTDT, DSSTDTC),
...
The `adsl` dataset:
```{r, eval=TRUE, echo=FALSE}
dataset_vignette(adsl, display_vars = get_admiral_option("subject_keys"), EOSDT))
```
...
```{r, eval=TRUE, echo=FALSE}
dataset_vignette(adsl, display_vars = get_admiral_option("subject_keys"), EOSDT, EOSSTT))
```
...
```{r, eval=TRUE, echo=FALSE}
dataset_vignette(adsl, display_vars = get_admiral_option("subject_keys"), EOSDT, EOSSTT, DCSREAS, DCSREASP))
```
...
```{r, eval=TRUE, echo=FALSE}
dataset_vignette(adsl, display_vars = get_admiral_option("subject_keys"), RANDDT))
```
...
```{r, eval=TRUE, echo=FALSE}
dataset_vignette(adsl %>% filter(!is.na(DTHDT) | row_number() %% 50 == 0), display_vars = get_admiral_option("subject_keys"), TRTEDT, DTHDTC, DTHDT, DTHFL))
```
...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), EOSDT, DTHDTC, DTHDT, DTHCAUS),
  filter = DTHFL == "Y"
)
```
...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), TRTEDT, DTHDTC, DTHDT, DTHCAUS, DTHDOM, DTHSEQ),
  filter = DTHFL == "Y"
)
```
...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), TRTEDT, DTHDTC, DTHDT, DTHCAUS, DTHADY, LDDTHELD),
  filter = DTHFL == "Y"
)
```
...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), TRTEDT, DTHDTC, LSTALVDT),
  filter = !is.na(TRTSDT)
)
```
...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), TRTEDT, DTHDTC, LSTALVDT, LALVDOM, LALVSEQ, LALVVAR),
  filter = !is.na(TRTSDT)
)
```
...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), AGE, SEX, COUNTRY, AGEGR1, REGION1)
)
```
...
dataset_vignette(
  adsl,
  display_vars = get_admiral_option("subject_keys"), TRTSDT, ARM, ACTARM, SAFFL)
)
```
