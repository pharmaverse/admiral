...
To derive the End of Study status (`EOSSTT`), this function must be provided with the mapping derivation for the `cat_fun` argument. The call below has the following mapping derivation:
...
```{r eval=TRUE}
# Setting up the values of the source variable as input for the cat_fun argument in derive_var_merged_cat function
format_dsdecod <- function(dsdecod_cat) {
  case_when(
    dsdecod_cat %in% c("SCREEN FAILURE", "SCREENING NOT COMPLETED") ~ "NOT STARTED",
    dsdecod_cat %in% "COMPLETED" ~ "COMPLETED",
    dsdecod_cat != "COMPLETED" | is.na(dsdecod_cat) ~ "DISCONTINUED",
    TRUE ~ "ONGOING"
  )
}
```
...
The function expects a mapping derivation for the `cat_fun` argument to derive `EOSSTT`.
The mapping derivation for the call above is
