...
head(adsl %>% select(USUBJID, starts_with("DTH")) %>% filter(DTHFL == "Y"))
```
...
adsl <- adsl %>%
  select(-DTHCAUS) %>% # remove it before calling again the function
  derive_var_dthcaus(src_ae, src_ds)
...
head(adsl %>% select(USUBJID, starts_with("DTH"), LDDTHELD) %>% filter(DTHFL == "Y"))
```
...
<b> <span style="color: red">link not working </span> </b>
