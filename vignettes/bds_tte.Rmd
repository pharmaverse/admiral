...
dataset_vignette(
  adtte,
  display_vars = c(get_admiral_option("subject_keys"), exprs(PARAMCD, PARAM, STARTDT, ADT, CNSR))
)
```
...
dataset_vignette(
  adsl,
  display_vars = c(get_admiral_option("subject_keys"), exprs(DTHFL, DTHDT, TRTSDT, TRTSDTF))
)
```
...
adrs <- tibble::tribble(
  ...
  select(!!!get_admiral_option("subject_keys"), PARAMCD, PARAM, ADT, ASEQ, AVALC)


dataset_vignette(
  adrs,
  display_vars = c(get_admiral_option("subject_keys"), exprs(AVALC, ADT, ASEQ, PARAMCD, PARAM))
)
```
...
dataset_vignette(
  adtte %>%
    select(
      !!!get_admiral_option("subject_keys"), PARAMCD, PARAM, STARTDT, ADT, ADTF, CNSR,
      EVNTDESC, SRCDOM, SRCVAR
    ),
  display_vars = c(get_admiral_option("subject_keys"), exprs(PARAMCD, STARTDT, ADT, ADTF, CNSR))
)
```
...
```{r, echo=FALSE}
adaette %>%
  select(!!!get_admiral_option("subject_keys"), PARAMCD, STARTDT, ADT, CNSR, EVNTDESC, SRCDOM, SRCVAR) %>%
  arrange(!!!get_admiral_option("subject_keys"), PARAMCD) %>%
  dataset_vignette(display_vars = c(get_admiral_option("subject_keys"), exprs(PARAMCD, STARTDT, ADT, CNSR, EVNTDESC, SRCDOM, SRCVAR)))
```
...
dataset_vignette(
  adtte %>%
    select(
      !!!get_admiral_option("subject_keys"), STARTDT, PARAMCD, PARAM, PARCAT1, PARCAT2, ADT, CNSR,
      EVNTDESC, SRCDOM, SRCVAR, SRCSEQ
    ),
  display_vars = c(get_admiral_option("subject_keys"), exprs(STARTDT, PARAMCD, PARAM, ADT, CNSR, SRCSEQ))
)
```
...
dataset_vignette(
  adtte,
  display_vars = c(get_admiral_option("subject_keys"), exprs(PARAMCD, CNSR, AVAL, ARMCD, AGE, SEX))
)
```
