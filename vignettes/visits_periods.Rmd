...
## The Period Reference Dataset {#reference}

The period reference dataset should be created according to the design and ADaM needs of the study in question. Generally it can be created using standard tidyverse commands for the exposure dataset of study.
...
## Example Creation of the Period Reference Dataset {#example_period}
...
* Phase 2 - Follow-up (Week 28 Onwards)

Consider the following partial exposure dataset:
...
Then to create a reference dataset for the periods, code like this (or similar) would suffice:
...
The workflow above populates `APEREDT` for all periods bar the last one. The value of the last period could then be populated with the End of Study Date (`EOSDT`) from ADSL.

A very similar process could be followed for the reference dataset for the study phases, this time using Day 1 and Week 28 as reference visits, to obtain:
...
phase_ref1 <- ex %>% 
  ...
  # Create APHASE, PHSDT based on SDTM counterparts
  mutate(
    APHASE = case_when(
      VISIT == "Day 1" ~ 1,
      VISIT == "Week 28" ~ 2
    ),
    PHSDT = lubridate::ymd(EXSTDTC)
  ) %>% 
