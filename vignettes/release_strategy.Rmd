...
1) Verify that all CI/CD checks are passing for the `devel` into the `main` Pull Request, merge and then bundle up and send off to CRAN.
1) If CRAN asks for modifications, repeat steps 1-2 as necessary.
1) Once the package is accepted and available on CRAN, a GitHub action is set up to rebuild the `{admiral}` website with all the updates for this release.
...
1) Create a Pull Request from `patch` into the `main` branch.  Verify that all CI/CD checks are passing, merge and bundle up and send off to CRAN.
