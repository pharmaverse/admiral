...
If a dataset needs to be created for testing purpose, it should be done so using the function `tribble()` from the `tibble` package with the following command `tibble::tribble(<data here>)`. If additional functions from other packages need to be called, it should be done using the notation `pkg_name::fun()`. 
