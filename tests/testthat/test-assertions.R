...
## Test 10: error if not a character scaler string ----
test_that("assert_character_scalar Test 10: error if not a character scaler string", {
...
## Test 11: error if input is a vector ----
test_that("assert_character_scalar Test 11: error if input is a vector", {
  example_fun2 <- function(msg_type) {
    msg_type <- assert_character_scalar(msg_type,
      values = c("warning", "error"), case_sensitive = FALSE
    )
...
# assert_vars ----
## Test 12: no error if expected input ----
test_that("assert_vars Test 12: no error if expected input", {
...
## Test 13: error if unexpected input ----
test_that("assert_vars Test 13: error if unexpected input", {
  expect_error(
    assert_vars(AVAL + 1),
    class = "assert_vars"
  )
...
# assert_data_frame ----
## Test 14: no error if optional is TRUE and `arg` is NULL ----
test_that("assert_data_frame Test 14: no error if optional is TRUE and `arg` is NULL", {
  example_fun <- function(dataset) {
...
## Test 15: error if required variables are missing ----
test_that("assert_data_frame Test 15: error if required variables are missing", {
  example_fun <- function(dataset) {
    assert_data_frame(dataset, required_vars = exprs(STUDYID, USUBJID))
  }
...
## Test 16: error if required variable is missing ----
test_that("assert_data_frame Test 16: error if required variable is missing", {
  example_fun <- function(dataset) {
    assert_data_frame(dataset, required_vars = exprs(STUDYID, USUBJID))
  }
...
# assert_character_scalar ----
## Test 17: no error if optional is TRUE and `arg` is NULL ----
test_that("assert_character_scalar Test 17: no error if optional is TRUE and `arg` is NULL", {
...
## Test 18: no error, case_sensitive = FALSE ----
test_that("assert_character_scalar Test 18: no error, case_sensitive = FALSE", {
  example_fun <- function(character) {
...
## Test 19: error if `arg` not in values ----
test_that("assert_character_scalar Test 19: error if `arg` not in values", {
  example_fun <- function(character) {
...
## Test 20: error if `arg` not a character vector ----
test_that("assert_character_vector Test 20: error if `arg` not a character vector", {
  arg <- c(1, 2, 3)
...
## Test 21: error if `arg` is not in values ----
test_that("assert_character_vector Test 21: error if `arg` is not in values", {
  example_fun <- function(character) {
...
## Test 22: arg_name correctly displayed in name check ----
test_that("assert_character_vector Test 22: arg_name correctly displayed in name check", {
  example_fun <- function(character) {
...
## Test 23: no error if optional is TRUE and `arg` is NULL ----
test_that("assert_logical_scalar Test 23: no error if optional is TRUE and `arg` is NULL", {
...
## Test 24: error if `arg` is not TRUE or FALSE ----
test_that("assert_logical_scalar Test 24: error if `arg` is not TRUE or FALSE", {
  example_fun <- function(arg) {
...
# assert_symbol ----
## Test 25: no error if optional = TRUE and `arg` = NULL ----
test_that("assert_symbol Test 25: no error if optional = TRUE and `arg` = NULL", {
  f <- function(var) {
    v <- enexpr(var)
  }
...
## Test 26: `assert_symbol` throws an error if `arg` is missing ----
test_that("assert_symbol Test 26: `assert_symbol` throws an error if `arg` is missing", {
  f <- function(var) {
    v <- enexpr(var)
  }
...
## Test 27: `assert_symbol` throws an error if `arg` is not a symbol ----
test_that("assert_symbol Test 27: `assert_symbol` throws an error if `arg` is not a symbol", {
  f <- function(var) {
    v <- enexpr(var)
  }
...
## Test 28: `assert_symbol` does not throw an error if `arg` is a symbol ----
test_that("assert_symbol Test 28: `assert_symbol` does not throw an error if `arg` is a symbol", {
  f <- function(var) {
    v <- enexpr(var)
  }
...
## Test 30: no error if optional is TRUE and `arg` is NULL ----
test_that("assert_expr Test 30: no error if optional is TRUE and `arg` is NULL", {
  expect_invisible(
    assert_expr(var <- NULL, optional = TRUE)
  )
})

## Test 31: `assert_expr` throws an error if `arg` is missing ----
test_that("assert_expr Test 31: `assert_expr` throws an error if `arg` is missing", {
  expect_error(
    assert_expr(),
    class = "assert_expr"
  )
...
## Test 32: `assert_expr` throws an error if `arg` is not an expression ----
test_that("assert_expr Test 32: `assert_expr` throws an error if `arg` is not an expression", {
  expect_error(
    {
      var <- c(1, 2)
      assert_expr(var)
...
# assert_vars ----
## Test 33: no error if `arg` is not a list of unquoted variable names ----
test_that("assert_vars Test 33: no error if `arg` is not a list of unquoted variable names", {
  example_fun <- function(arg) {
    assert_vars(arg)
  }
...
## Test 34: error if some elements of `arg` are not unquoted variable names ----
test_that("assert_vars Test 34: error if some elements of `arg` are not unquoted variable names", {
  example_fun <- function(arg) {
    assert_vars(arg)
  }
...
## Test 35: no error if `arg` is NULL and optional is TRUE ----
test_that("assert_integer_scalar Test 35: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
...
## Test 36: error if chosen subset not in subsets ----
test_that("assert_integer_scalar Test 36: error if chosen subset not in subsets", {
  example_fun <- function(arg) {
...
## Test 37: no error if `arg` is in selected subset ----
test_that("assert_integer_scalar Test 37: no error if `arg` is in selected subset", {
  example_fun <- function(arg) {
...
test_that("assert_numeric_vector Test 39: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
...
# assert_named_exprs ----
## Test 54: error if `arg` is not a named list of expressions (deprecation error) ----
test_that("assert_named_exprs Test 54: error if `arg` is not a named list of expressions (deprecation error)", { # nolint
  example_fun <- function(arg) {
...
## Test 55: no error if `arg` is NULL and optional is TRUE (deprecation error) ----
test_that("assert_named_exprs Test 55: no error if `arg` is NULL and optional is TRUE (deprecation error)", { # nolint
  example_fun <- function(arg) {
...
## Test 56: no error if `arg` is a named list of expressions (deprecation error) ----
test_that("assert_named_exprs Test 56: no error if `arg` is a named list of expressions (deprecation error)", { # nolint
  example_fun <- function(arg) {
    assert_named_exprs(arg)
  }
...
# assert_function ----
## Test 57: error if `arg` is not a function ----
test_that("assert_function Test 57: error if `arg` is not a function", {
  example_fun <- function(arg) {
    assert_function(arg)
  }
...
## Test 58: no error if `arg` is NULL and optional is TRUE ----
test_that("assert_function Test 58: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
...
## Test 59: no error if `arg` is a function with all parameters defined ----
test_that("assert_function Test 59: no error if `arg` is a function with all parameters defined", {
  example_fun <- function(arg) {
...
## Test 60: error if  `params`  is missing with no default ----
test_that("assert_function Test 60: error if  `params`  is missing with no default", {
  example_fun <- function(arg) {
    assert_function(arg, params = c("x"))
  }
...
## Test 61: If dot-dot-dot is a argument ----
test_that("assert_function Test 61: If dot-dot-dot is a argument", {
...
## Test 62: no error if `arg` is a parameter of a function (deprecation error) ----
test_that("assert_function_param Test 62: no error if `arg` is a parameter of a function (deprecation error)", { # nolint
  hello <- function(name) {
...
## Test 63: error if expected function parameters are missing (deprecation error) ----
test_that("assert_function_param Test 63: error if expected function parameters are missing (deprecation error)", { # nolint
  hello <- function(name) {
    print(sprintf("Hello %s", name))
  }
...
## Test 70: error if `arg` is not a list of var-value expressions ----
test_that("assert_varval_list Test 70: error if `arg` is not a list of var-value expressions", {
  example_fun <- function(arg) {
...
## Test 71: error if `required_elements` are missing from `arg` ----
test_that("assert_varval_list Test 71: error if `required_elements` are missing from `arg`", {
  example_fun <- function(arg) {
...
## Test 72: no error if `arg` is NULL and optional is TRUE ----
test_that("assert_varval_list Test 72: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
...
## Test 73: error if `accept_expr` is TRUE and value is invalid ----
test_that("assert_varval_list Test 73: error if `accept_expr` is TRUE and value is invalid", {
  example_fun <- function(arg) {
...
## Test 74: error if `accept_expr` is FALSE and value is invalid ----
test_that("assert_varval_list Test 74: error if `accept_expr` is FALSE and value is invalid", {
  example_fun <- function(arg) {
...
## Test 75: no error if an argument is a variable-value list ----
test_that("assert_varval_list Test 75: no error if an argument is a variable-value list", {
  example_fun <- function(arg) {
...
# assert_expr_list ----
## Test 76: error if `arg` is not a list of expressions ----
...
## Test 77: error if `arg` is not a named list of expressions ----
test_that("assert_expr_list Test 77: error if `arg` is not a named list of expressions", {
  expect_error(
...
## Test 78: error if `required_elements` are missing from `arg` ----
test_that("assert_expr_list Test 78: error if `required_elements` are missing from `arg`", {
  expect_error(
    assert_expr_list(
...
## Test 79: no error if `arg` is NULL and optional is TRUE ----
test_that("assert_expr_list Test 79: no error if `arg` is NULL and optional is TRUE", {
  expect_invisible(
    assert_expr_list(NULL, optional = TRUE)
  )
})

## Test 80: error if element is invalid ----
...
## Test 81: no error if argument is valid ----
test_that("assert_expr_list Test 81: no error if argument is valid", {
  expect_invisible(
...
test_that("assert_list_element Test 82: no error if the elements fulfill a certain condition", {
  expect_invisible(
    assert_list_element(
      list(
...
## Test 83: error if the elements do not fulfill the condition ----
