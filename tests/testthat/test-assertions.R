...
# assert_named_exprs ----
## Test 47: error if `arg` is not a named list of expressions ----
test_that("assert_named_exprs Test 47: error if `arg` is not a named list of expressions", {
  example_fun <- function(arg) {
...
test_that("assert_named_exprs Test 48: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
...
## Test 49: no error if `arg` is a named list of expressions ----
test_that("assert_named_exprs Test 49: no error if `arg` is a named list of expressions", {
  example_fun <- function(arg) {
    assert_named_exprs(arg)
  }
...
# assert_function ----
## Test 50: error if `arg` is not a function ----
test_that("assert_function Test 50: error if `arg` is not a function", {
  example_fun <- function(arg) {
    assert_function(arg)
  }
...
## Test 51: no error if `arg` is NULL and optional is TRUE ----
test_that("assert_function Test 51: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
...
## Test 52: no error if `arg` is a function with all parameters defined ----
test_that("assert_function Test 52: no error if `arg` is a function with all parameters defined", {
  example_fun <- function(arg) {
...
## Test 53: error if  `params`  is missing with no default ----
test_that("assert_function Test 53: error if  `params`  is missing with no default", {
  example_fun <- function(arg) {
    assert_function(arg, params = c("x"))
  }
...
## Test 54: no error if `arg` is a parameter of a function ----
test_that("assert_function_param Test 54: no error if `arg` is a parameter of a function", {
  hello <- function(name) {
...
## Test 55: error if expected function parameters are missing ----
test_that("assert_function_param Test 55: error if expected function parameters are missing", {
  hello <- function(name) {
    print(sprintf("Hello %s", name))
  }
...
## Test 57: error if there are multiple units in the input dataset ----
test_that("assert_unit Test 57: error if there are multiple units in the input dataset", {
  advs <- tibble::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU, ~PARAMCD, ~AVAL,
    "P01",    "WEIGHT",      80.1, "kg",      "WEIGHT",  80.1,
...
## Test 58: error if unexpected unit in the input dataset ----
test_that("assert_unit Test 58: error if unexpected unit in the input dataset", {
  advs <- tibble::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU, ~PARAMCD, ~AVAL,
    "P01",    "WEIGHT",      80.1, "kg",      "WEIGHT",  80.1,
    "P02",    "WEIGHT",      85.7, "kg",      "WEIGHT",  85.7
...
## Test 59: error if parameter exists in the input dataset ----
test_that("assert_param_does_not_exist Test 59: error if parameter exists in the input dataset", {
  advs <- tibble::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU, ~PARAMCD, ~AVAL,
    "P01",    "WEIGHT",      80.1, "kg",      "WEIGHT",  80.1,
    "P02",    "WEIGHT",      85.7, "kg",      "WEIGHT",  85.7
...
## Test 60: no error if the parameter exists in the dataset ----
test_that("assert_param_does_not_exist Test 60: no error if the parameter exists in the dataset", {
  advs <- tibble::tribble(
    ~USUBJID, ~VSTESTCD, ~VSTRESN, ~VSSTRESU, ~PARAMCD, ~AVAL,
    "P01",    "WEIGHT",      80.1, "kg",      "WEIGHT",  80.1,
    "P02",    "WEIGHT",      85.7, "kg",      "WEIGHT",  85.7
...
## Test 62: error if `arg` is not a list of var-value expressions ----
test_that("assert_varval_list Test 62: error if `arg` is not a list of var-value expressions", {
  example_fun <- function(arg) {
...
## Test 63: error if `required_elements` are missing from `arg` ----
test_that("assert_varval_list Test 63: error if `required_elements` are missing from `arg`", {
  example_fun <- function(arg) {
...
## Test 64: no error if `arg` is NULL and optional is TRUE ----
test_that("assert_varval_list Test 64: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
...
## Test 65: error if `accept_expr` is TRUE and value is invalid ----
test_that("assert_varval_list Test 65: error if `accept_expr` is TRUE and value is invalid", {
  example_fun <- function(arg) {
...
## Test 66: error if `accept_expr` is FALSE and value is invalid ----
test_that("assert_varval_list Test 66: error if `accept_expr` is FALSE and value is invalid", {
  example_fun <- function(arg) {
...
## Test 67: no error if an argument is a variable-value list ----
test_that("assert_varval_list Test 67: no error if an argument is a variable-value list", {
  example_fun <- function(arg) {
...
# assert_list_element ----
## Test 68: no error if the elements fulfill a certain condition ----
test_that("assert_list_element Test 68: no error if the elements fulfill a certain condition", {
  expect_invisible(
...
## Test 69: error if the elements do not fulfill the condition ----
test_that("assert_list_element Test 69: error if the elements do not fulfill the condition", {
  expect_error(
    assert_list_element(vars(DTHDOM = "AE", DTHSEQ = admiral.test::admiral_dm), "DTHSEQ",
...
# assert_one_to_one ----
## Test 70: error if there is a one to many mapping ----
test_that("assert_one_to_one Test 70: error if there is a one to many mapping", {
  expect_error(
    assert_one_to_one(admiral.test::admiral_dm, vars(DOMAIN), vars(USUBJID))
  )
})

## Test 71: error if there is a many to one mapping ----
test_that("assert_one_to_one Test 71: error if there is a many to one mapping", {
  expect_error(
...
# assert_date_var ----
## Test 72: error if variable is not a date or datetime variable ----
test_that("assert_date_var Test 72: error if variable is not a date or datetime variable", {
...
# assert_date_vector ----
## Test 73: returns error if input vector is not a date formatted ----
test_that("assert_date_vector Test 73: returns error if input vector is not a date formatted", {
  expect_error(assert_date_vector("2018-08-23"))
})

## Test 74: returns invisible if input is date formatted ----
test_that("assert_date_vector Test 74: returns invisible if input is date formatted", {
  expect_invisible(assert_date_vector(as.Date("2022-10-25")))
})

## Test 75: no error if `arg` is NULL and optional is TRUE ----
test_that("assert_date_vector Test 75: no error if `arg` is NULL and optional is TRUE", {
  example_fun <- function(arg) {
