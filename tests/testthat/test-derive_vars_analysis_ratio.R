test_that("Test 1: All Ratio Variables are Created", {
 ...
 actual_data <- data %>% derive_vars_analysis_ratio(ratio_vars = "all")
...
test_that("Test 3: When selecting ratio_vars = low only R2BASE and R2ANRLO
          variables are selected while NAs present", {
  expected_data <- tibble::tribble(
    ~USUBJID, ~PARAMCD, ~SEQ, ~AVAL, ~BASE, ~ANRLO, ~ANRHI, ~R2BASE, ~R2ANRLO,
    "P01",    "ALT",    1,    27,    27,    6,      34,     1,        4.5,
    "P01",    "ALT",    2,    41,    27,    6,      34,     1.52,    6.83,
    "P01",    "ALT",    3,    NA_real_,27,  6,      34,     NA_real_,NA_real_,
    "P02",    "ALB",    1,    38,    38,    33,     49,     1,        1.15,
    "P02",    "ALB",    2,    39,    38,    33,     49,     1.03,     1.18,
    "P02",    "ALB",    3,    37,    38,    33,     49,     0.974,    1.12,
  )
  ...
  actual_data <- data %>% derive_vars_analysis_ratio(ratio_vars = "low")
