...
test_that("derive_var_basetype Test 1: deprecation warning if function is called", {
...
## Test 2: records are duplicated across different `BASETYPE` values ----
test_that("derive_var_basetype Test 2: records are duplicated across different `BASETYPE` values", {
  input <- tibble::tribble(
    ~USUBJID, ~EPOCH,         ~PARAMCD,  ~ASEQ, ~AVAL,
    "P01",    "RUN-IN",       "PARAM01",     1,  10.0,
    "P01",    "RUN-IN",       "PARAM01",     2,   9.8,
  ...
  actual_output <- suppress_warning(
    derive_var_basetype(
      dataset = input,
      basetypes = rlang::exprs(
        "RUN-IN" = EPOCH %in% c("RUN-IN", "STABILIZATION", "DOUBLE-BLIND", "OPEN-LABEL"),
...
## Test 3: records that do not match any condition are kept ----
test_that("derive_var_basetype Test 3: records that do not match any condition are kept", {
  input <- tibble::tribble(
    ~USUBJID, ~EPOCH,         ~PARAMCD,  ~ASEQ, ~AVAL,
