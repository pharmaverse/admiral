...
test_that("compute_scale Test 1: compute_scale works as expected", {
  ...
  expect_equal(compute_scale(input, c(1, 5), c(0, 100), min_n = 3), expected_output)
...
test_that("compute_scale Test 2: scale is flipped if flip_direction == TRUE", {
  ...
  input <- adqs %>%
    filter(USUBJID == "01-701-1015" & AVISITN == 200 & PARAMCD %in% c("ITEM5", "ITEM6", "ITEM7")) %>%
    pull()

  expected_output <- 100 - (mean((input - 1) * 50, na.rm = TRUE))

  expect_equal(compute_scale(input, c(1, 3), c(0, 100), flip_direction = TRUE, min_n = 3), expected_output)
...
test_that("compute_scale Test 3: result is missing if min_n is not met", {
  ...
  expect_equal(compute_scale(input, c(1, 5), c(0, 100), min_n = 2), expected_output)

})

## Test 4: compute_scale() works as expected within derive_summary_records() ----
test_that("compute_scale Test 4: compute_scale() works as expected within derive_summary_records()", {
  ...
  expect_equal(derive_summary_records(
                ...
                filter = (PARAMCD %in% c("ITEM1", "ITEM2", "ITEM3")),
                analysis_var = AVAL,
                summary_fun = function(x) compute_scale(x, c(1, 5), c(0, 100), flip_direction = TRUE, min_n = 3),
