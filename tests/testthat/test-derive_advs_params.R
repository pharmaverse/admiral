...
test_that("compute_bsa Test 02.01: DuBois-DuBois method - single height and weight values", {
  expect_equal(
    ...
    0.20247 * (170 / 100)^0.725 * 75^0.425
  )
})
...
test_that("compute_bsa Test 02.03: DuBois-DuBois method - height and weight vectors - missing values", {
  expect_equal(
    compute_bsa(
      height = c(NA, 185),
      weight = c(75, NA),
...
test_that("compute_bsa Test 04.03: Gehan-George method - height and weight vectors - missing values", {
  expect_equal(
    compute_bsa(
      height = c(NA, 185),
      weight = c(75, NA),
...
test_that("compute_bsa Test 05.01: Boyd method - single height and weight values", {
  expect_equal(
    ...
    0.0003207 * (170^0.3) *
      (1000 * 75) ^ (0.7285 - (0.0188 * log10(1000 * 75)))
  )
})
...
test_that("compute_map Test 01.01: Mean Arterial Pressure based on diastolic and systolic blood pressure - single values", {
  expect_equal(
    compute_map(
      diabp = 51,
      sysbp = 121
...
test_that("compute_map Test 01.02: Mean Arterial Pressure based on diastolic and systolic blood pressure - vectors", {
  expect_equal(
    compute_map(
      diabp = c(51, 61),
      sysbp = c(121, 141)
    ),
  (2 * c(51,61) + c(121,141)) / 3
  )
})

test_that("compute_map Test 01.03: Mean Arterial Pressure based on diastolic and systolic blood pressure - vectors with missing values", {
  expect_equal(
    compute_map(
      diabp = c(NA, 61),
      sysbp = c(121, NA)
...
test_that("compute_map Test 02.01: Mean Arterial Pressure based on diastolic, systolic blood pressure and heart rate - single values", {
  expect_equal(
    compute_map(
      diabp = 51,
      sysbp = 121,
...
test_that("compute_map Test 02.02: Mean Arterial Pressure based on diastolic, systolic blood pressure and heart rate - vectors", {
  expect_equal(
    compute_map(
      diabp = c(51, 91),
      sysbp = c(121, 91),
    ...
    c(51,91) + 0.01 * exp(4.14 - 40.74 / c(59,62)) * (c(121,91) - c(51,91))
  )
})

test_that("compute_map Test 02.03: Mean Arterial Pressure based on diastolic, systolic blood pressure and heart rate - vectors with missing values", {
  expect_equal(
    compute_map(
      diabp = c(NA, 61, 51),
      sysbp = c(121, NA, 121),
...
test_that("derive_param_bmi Test 01.01: BMI parameter NOT added to input dataset - wrong unit for height", {
...
test_that("derive_param_bmi Test 01.02: BMI parameter NOT added to input dataset - wrong unit for weight", {
  input <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
...
test_that("derive_param_bmi Test 01.03: BMI parameter NOT added to input dataset - multiple unit for weight", {
  input <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
...
test_that("derive_param_bmi Test 01.04: BMI parameter NOT added to input dataset - PARAMCD not set", {
...
test_that("derive_param_bmi Test 03.01: BMI parameter is correctly added to input dataset", {
  expected_output <- tibble::tribble(
    ...
    "01-701-1015", "BMI",               NA, "BASELINE",        NA,  75/(170/100)^2,
    "01-701-1015", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    78,
    ...
    "01-701-1028", "BMI",               NA, "BASELINE",        NA,  90/(185/100)^2,
...
test_that("derive_param_bsa Test 01.01: BSA parameter NOT added to input dataset - wrong unit for height", {
  input <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    # Wrong unit for HEIGHT should be cm
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "m",   170,
...
test_that("derive_param_bsa Test 01.02: BSA parameter NOT added to input dataset - wrong unit for weight", {
  input <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    # Wrong unit for WEIGHT should be kg
...
test_that("derive_param_bsa Test 01.03: BSA parameter NOT added to input dataset - multiple unit for weight", {
  input <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    # Multiple units for WEIGHT
...
test_that("derive_param_bsa Test 01.04: BSA parameter NOT added to input dataset - PARAMCD not set", {
...
test_that("derive_param_bsa Test 03.01: BSA parameter (Mosteller method) is correctly added to input dataset", {
  expected_output <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    "01-701-1015", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    75,
    # New row added for BMI for SUBJID="01-701-1015" and VISIT="BASELINE"
    # WEIGHT = 75 and HEIGHT = 170
    "01-701-1015", "BSA",               NA, "BASELINE",        NA,  sqrt(170*75/3600),
    "01-701-1015", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    78,
    "01-701-1028", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   185,
    "01-701-1028", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    90,
    # New row added for BMI for SUBJID="01-701-1028" and VISIT='BASELINE'
    # WEIGHT = 90 and HEIGHT = 185
    "01-701-1028", "BSA",               NA, "BASELINE",        NA,  sqrt(185*90/3600),
    "01-701-1028", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    88,
  )
...
test_that("derive_param_bsa Test 03.02: BSA parameter (DuBois-DuBois method) is correctly added to input dataset", {
  expected_output <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    "01-701-1015", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    75,
    # New row added for BMI for SUBJID="01-701-1015" and VISIT="BASELINE"
    # WEIGHT = 75 and HEIGHT = 170
    "01-701-1015", "BSA",               NA, "BASELINE",        NA,  0.20247*(170/100)^0.725*75^0.425,
    "01-701-1015", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    78,
    "01-701-1028", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   185,
    "01-701-1028", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    90,
    # New row added for BMI for SUBJID="01-701-1028" and VISIT='BASELINE'
    # WEIGHT = 90 and HEIGHT = 185
    "01-701-1028", "BSA",               NA, "BASELINE",        NA,  0.20247*(185/100)^0.725*90^0.425,
    "01-701-1028", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    88,
  )
...
test_that("derive_param_bsa Test 03.03: BSA parameter (Haycock method) is correctly added to input dataset", {
  expected_output <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    "01-701-1015", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    75,
    # New row added for BMI for SUBJID="01-701-1015" and VISIT="BASELINE"
    # WEIGHT = 75 and HEIGHT = 170
    "01-701-1015", "BSA",               NA, "BASELINE",        NA,  0.024265 * 170^0.3964 * 75^0.5378,
    "01-701-1015", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    78,
    "01-701-1028", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   185,
    "01-701-1028", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    90,
    # New row added for BMI for SUBJID="01-701-1028" and VISIT='BASELINE'
    # WEIGHT = 90 and HEIGHT = 185
    "01-701-1028", "BSA",               NA, "BASELINE",        NA,  0.024265 * 185^0.3964 * 90^0.5378,
    "01-701-1028", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    88,
  )
...
test_that("derive_param_bsa Test 03.04: BSA parameter (Gehan-George method) is correctly added to input dataset", {
  expected_output <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    "01-701-1015", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    75,
    # New row added for BMI for SUBJID="01-701-1015" and VISIT="BASELINE"
    # WEIGHT = 75 and HEIGHT = 170
    "01-701-1015", "BSA",               NA, "BASELINE",        NA,  0.0235 * 170^0.42246 * 75^0.51456,
    "01-701-1015", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    78,
    "01-701-1028", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   185,
    "01-701-1028", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    90,
    # New row added for BMI for SUBJID="01-701-1028" and VISIT='BASELINE'
    # WEIGHT = 90 and HEIGHT = 185
    "01-701-1028", "BSA",               NA, "BASELINE",        NA,  0.0235 * 185^0.42246 * 90^0.51456,
    "01-701-1028", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    88,
  )
...
test_that("derive_param_bsa Test 03.05: BSA parameter (Boyd method) is correctly added to input dataset", {
  expected_output <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    "01-701-1015", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    75,
    # New row added for BMI for SUBJID="01-701-1015" and VISIT="BASELINE"
    # WEIGHT = 75 and HEIGHT = 170
    "01-701-1015", "BSA",               NA, "BASELINE",        NA,  0.0003207*(170^0.3)*(1000*75)^(0.7285 - (0.0188*log10(1000*75))),
    "01-701-1015", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    78,
    "01-701-1028", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   185,
    "01-701-1028", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    90,
    # New row added for BMI for SUBJID="01-701-1028" and VISIT='BASELINE'
    # WEIGHT = 90 and HEIGHT = 185
    "01-701-1028", "BSA",               NA, "BASELINE",        NA,  0.0003207*(185^0.3)*(1000*90)^(0.7285 - (0.0188*log10(1000*90))),
    "01-701-1028", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    88,
  )
...
test_that("derive_param_bsa Test 03.06: BSA parameter (Fujimoto method) is correctly added to input dataset", {
  expected_output <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    "01-701-1015", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    75,
    # New row added for BMI for SUBJID="01-701-1015" and VISIT="BASELINE"
    # WEIGHT = 75 and HEIGHT = 170
    "01-701-1015", "BSA",               NA, "BASELINE",        NA,  0.008883 * 170^0.663 * 75^0.444,
    "01-701-1015", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    78,
    "01-701-1028", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   185,
    "01-701-1028", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    90,
    # New row added for BMI for SUBJID="01-701-1028" and VISIT='BASELINE'
    # WEIGHT = 90 and HEIGHT = 185
    "01-701-1028", "BSA",               NA, "BASELINE",        NA,  0.008883 * 185^0.663 * 90^0.444,
    "01-701-1028", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    88,
  )
...
test_that("derive_param_bsa Test 03.07: BSA parameter (Takahira method) is correctly added to input dataset", {
  expected_output <- tibble::tribble(
    ~USUBJID,      ~PARAMCD, ~PARAM,        ~VISIT,     ~VSSTRESU, ~AVAL,
    "01-701-1015", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   170,
    "01-701-1015", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    75,
    # New row added for BMI for SUBJID="01-701-1015" and VISIT="BASELINE"
    # WEIGHT = 75 and HEIGHT = 170
    "01-701-1015", "BSA",               NA, "BASELINE",        NA,  0.007241 * 170^0.725 * 75^0.425,
    "01-701-1015", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    78,
    "01-701-1028", "HEIGHT", "Height (cm)", "BASELINE",      "cm",   185,
    "01-701-1028", "WEIGHT", "Weight (kg)", "BASELINE",      "kg",    90,
    # New row added for BMI for SUBJID="01-701-1028" and VISIT='BASELINE'
    # WEIGHT = 90 and HEIGHT = 185
    "01-701-1028", "BSA",               NA, "BASELINE",        NA,  0.007241 * 185^0.725 * 90^0.425,
    "01-701-1028", "WEIGHT", "Weight (kg)",  "MONTH 1",      "kg",    88,
  )
...
test_that("derive_param_map Test 01.01: MAP parameter NOT added to input dataset - wrong unit for DIABP", {
...
test_that("derive_param_map Test 01.02: MAP parameter NOT added to input dataset - wrong unit for SYSBP", {
...
test_that("derive_param_map Test 01.03: MAP parameter NOT added to input dataset - wrong unit for PULSE", {
...
test_that("derive_param_map Test 01.04: MAP parameter NOT added to input dataset - PARAMCD not set", {
...
test_that("derive_param_map Test 03.01: MAP parameter (DBP/SBP/PULSE) is correctly added to input dataset", {
  ...
  expected_output <- tibble::tribble(
    ...
    "01-701-1015",   "MAP",                                NA,  "BASELINE", 51 + 0.01 * exp(4.14 - 40.74 / 59) * (121 - 51),
    ...
    "01-701-1028",   "MAP",                                NA,  "WEEK 2",   50 + 0.01 * exp(4.14 - 40.74 / 61) * (125 - 50),
  )
...
test_that("derive_param_map Test 03.02: MAP parameter (DBP/SBP) is correctly added to input dataset", {
  ...
  expected_output <- tibble::tribble(
    ...
    "01-701-1015",   "MAP",                                NA,  "BASELINE", (2 * 51 + 121) / 3,
    "01-701-1028", "DIABP",  "Diastolic Blood Pressure (mmHg)", "WEEK 2",    50,
    "01-701-1028", "SYSBP",  "Systolic Blood Pressure (mmHg)",  "WEEK 2",   125,
    # New row added for MAP for SUBJID="01-701-1028" and VISIT="WEEK 2"
    # DIABP = 50 and SYSBP = 125
    "01-701-1028",   "MAP",                                NA,  "WEEK 2",   (2 * 50 + 125) / 3,
  )
