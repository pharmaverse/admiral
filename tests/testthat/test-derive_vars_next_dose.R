...
test_that("derive_vars_next_dose Test 1: function works as expected", {
  expected_output <- mutate(
    input_ae,
    EXSTDT = as.Date(c("2020-08-29", "2020-09-02", "2020-10-20", "2019-05-25", NA, "2020-03-15", NA)),
    EXENDT = as.Date(c("2020-08-29", "2020-09-02", "2020-10-20", "2019-05-25", NA, "2020-03-15", NA)),
    EXSEQ = c(2, 3, 4, 1, NA, 1, NA),
    EXDOSE = c(10, 10, 10, 0, NA, 10, NA),
    EXTRT = c("treatment", "treatment", "treatment", "placebo", NA, "treatment", NA)
  )

  res <- derive_vars_next_dose(
    input_ae,
    input_ex,
...
test_that("derive_vars_next_dose Test 2: function checks validity of start and end dose inputs", {
...
test_that("derive_vars_next_dose Test 3: function returns traceability vars", {
  expected_output <- mutate(
    input_ae,
    EXSTDTC = c("2020-08-29", "2020-09-02", "2020-10-20", "2019-05-25", NA, "2020-03-15", NA),
    EXENDTC = c("2020-08-29", "2020-09-02", "2020-10-20", "2019-05-25", NA, "2020-03-15", NA),
    EXENDT = ymd(EXENDTC),
    EXSTDT = ymd(EXSTDTC),
    EXSEQ = c(2, 3, 4, 1, NA, 1, NA),
    EXDOSE = c(10, 10, 10, 0, NA, 10, NA),
    EXTRT = c("treatment", "treatment", "treatment", "placebo", NA, "treatment", NA),
    NDOSEDOM = c("EX", "EX", "EX", "EX", NA, "EX", NA),
    NDOSESEQ = c(2, 3, 4, 1, NA, 1, NA),
    NDOSEVAR = c("EXSTDTC", "EXSTDTC", "EXSTDTC", "EXSTDTC", NA, "EXSTDTC", NA)
  )
  ...
  res <- derive_vars_next_dose(
    ...
    analysis_date = AESTDT,
    single_dose_condition = (EXSTDTC == EXENDTC),
    traceability_vars = vars(NDOSEDOM = "EX", NDOSESEQ = EXSEQ, NDOSEVAR = "EXSTDTC")
  )
...
test_that("derive_vars_next_dose Test 4: function errors when multiple doses are on same date", {
...
test_that("derive_vars_next_dose Test 5: multiple doses on same date - dose_id supplied", {
  input_ex_dup <- bind_rows(
    input_ex,
    tribble(
      ~STUDYID, ~USUBJID, ~EXSTDTC, ~EXENDTC, ~EXSEQ, ~EXDOSE, ~EXTRT,
  ...
  expected_output <- mutate(
    input_ae,
    EXSTDT = ymd(c("2020-08-29", "2020-09-02", "2020-10-20", "2019-05-25", NA, "2020-03-15", NA)),
    EXENDT = ymd(c("2020-08-29", "2020-09-02", "2020-10-20", "2019-05-25", NA, "2020-03-15", NA)),
    EXSEQ = c(2, 3, 4, 1, NA, 1, NA),
    EXDOSE = c(10, 10, 10, 0, NA, 10, NA),
    EXTRT = c("treatment", "treatment", "treatment", "placebo", NA, "treatment", NA)
  )
...
test_that("derive_vars_next_dose Test 6: error is issued if same variable is found in both input datasets", { # nolint
  input_ae <- tribble(
...
test_that("derive_vars_next_dose Test 7: no error is raised when setting `dose_date` to a renamed variable", { # nolint
  adae <- tribble(
