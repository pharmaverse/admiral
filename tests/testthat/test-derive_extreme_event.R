...
## Test 9: filter_source in event_joined() ----
test_that("derive_extreme_event Test 9: filter_source in event_joined()", {
  adbds <- tribble(
    ~USUBJID, ~AVISITN,  ~AVALC,
    "1",             1,  "Y",
    "1",             2,  "N",
  ...
  actual <- derive_extreme_event(
    ...
    events = list(
      ...
      event(
        dataset_name = "adbds",
        set_values_to = exprs(
          AVALC = if_else(!is.na(AVISITN) & !is.na(AVALC), "N", NA_character_)
        )
      )
    ),
    set_values_to = exprs(
