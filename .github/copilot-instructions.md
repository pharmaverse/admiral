...
**Style:**
- Follow the [tidyverse style guide](https://style.tidyverse.org/) with admiral-specific extensions in the Programming Strategy
- Use tidyverse packages (dplyr, tidyr, purrr, stringr, etc.) over base R equivalents
- Use `%>%` pipe operator for readability (the package currently uses magrittr pipe)
...
- Use existing test data from `pharmaversesdtm` package when possible (e.g., `pharmaversesdtm::dm`, `pharmaversesdtm::ex`)
...
- Roxygen2 automatically generates NAMESPACE with exports in alphabetical order
- Run `devtools::document()` to regenerate after adding/modifying `@export` tags
