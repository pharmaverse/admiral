#' General Function Derive Last Dose Date(-time)
#'
...
#'
derive_last_dose_gf <- function(dataset,
                             dataset_ex,
                             filter_ex = NULL,
  ...
  if (is.null(traceability_vars)) {
    ...
    res <- res %>%
      mutate(
        ldose_idx = compute_ldose_idx__gf(dose_end = .data$tmp_dose_end_date,
                                      analysis_date = .data$tmp_analysis_date),
        !!new_var := as.POSIXct(as.character(.data$tmp_dose_end_date[.data$ldose_idx]),
                                tz = lubridate::tz(.data$tmp_dose_end_date))
      ) %>%
...
compute_ldose_idx_gf <- function(dose_end, analysis_date) {
