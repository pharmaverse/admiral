#' Derive next Dose
#'
#' Add EX source variables from next dose to the input dataset.
#'
...
#' e.g. `vars(NDOSEDOM = "EX", NDOSESEQ = EXSEQ)`.
...
#'
#' @details
#' When doing date comparison to identify next dose, date-time imputations are done as follows:
#' * `dose_date`: time is imputed to `00:00:00` if the variable is a date variable
#' * `analysis_date`: time is imputed to `00:00:00` if the variable is a date variable
#'
#' The next dose records are identified as follows:
#'
...
#' 3. The next dose is identified:
#' the next dose is the EX record with maximum date where `dose_date` is greater than
#' `analysis_date`, subject to both date values are non-NA.
#' The next dose is identified per `by_vars`.
...
#' or an error is issued. When `dose_id` is supplied, the next EX record from the same `dose_date`
#' sorted by `dose_id` will be used to identify next dose.
#' 4. The EX source variables (as specified in `new_vars`) from next dose are appended to the
...
#'
#' @return Input dataset with EX source variables from next dose added.
#'
#' @author Ondrej Slama, Annie Yang, Jeff Dickinson
#'
#' @family der_gen
#' @keywords der_gen
#'
...
#'       !is.na(EXSTDTM),
#'     new_vars = vars(EXDOSE, EXTRT, EXSEQ, EXSTDTC, VISIT),
#'     dose_date = EXSTDTM,
#'     analysis_date = ADTM
#'   ) %>%
#'   select(STUDYID, USUBJID, PCSEQ, PCDTC, EXDOSE, EXTRT, EXSTDTC, EXSEQ, VISIT)
#'
...
#'     dose_date = EXSTDTM,
#'     analysis_date = ADTM,
#'     traceability_vars = dplyr::vars(NDOSEDOM = "EX", NDOSESEQ = EXSEQ, NDOSEVAR = "EXSTDTC")
#'   ) %>%
#'   select(STUDYID, USUBJID, PCSEQ, PCDTC, EXDOSE, EXTRT, EXSTDTC, NDOSEDOM, NDOSESEQ, NDOSEVAR)
derive_vars_next_dose <- function(dataset,
                                  dataset_ex,
                                  filter_ex = NULL,
  ...
  # create temporary observation number and temporary numeric date to identify next dose
  dataset <- dataset %>%
  ...
  # create tmp numeric date to enable date comparison for identifying next dose
  dataset_ex <- dataset_ex %>%
    mutate(
  ...
  # join datasets and keep unique next dose records (where dose_date is before or on analysis_date)
  res <- dataset %>%
  ...
  # return observations from original dataset with next dose variables added
  derive_vars_merged(
    dataset,
    dataset_add = res,
    by_vars = vars(tmp_seq_var)
