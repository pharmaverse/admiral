...
#'
#'   For observations of the reference dataset (`dataset_ref`) which do not have
#'   a matching record in `dataset_add` (with respect to `by_vars` and after
#'   applying `filter_add`), the specified variables are set to the specified
...
#'   arrange(USUBJID, AVISIT)
#'
#' @caption Filtering out missing summary records
#'
#' @info It may be helpful to print out missing records to determine
...
#' to all summary records generated by the function. This is particularly useful
#' when you need to populate new information for observations derived from the
#' `dataset_add` as well as for new records created for subjects present in
#' `dataset_ref` but missing in `dataset_add`.
#'
#' For example, if `ADSL` contains two subjects ("1" and "2"), but `ADAE` only
#' has adverse event information for "Subject 1", `derive_summary_records` will:
#'
#' 1. Create a summary record for "Subject 1" based on `ADAE`.
#' 2. Identify "Subject 2" (from `ADSL`) as having no corresponding records in
#' `ADAE` and create a new record for it.
#'
#' The `constant_values` argument ensures that all these generated summary records
#' (for both Subject 1 and the newly created record for Subject 2) receive the
#' same `PARAMCD`, `PARAM`, and `PARCAT1` values.
#' Additionally, `missing_values` is used to specifically set `AVAL` to 0 for
#' "Subject 2" when no adverse events are found.
#'
#'
#' @code
#' library(tibble)
