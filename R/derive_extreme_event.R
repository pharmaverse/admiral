#' Find the Worst or Best Observation for Each By Group as New Records
#'
#' This is a helper function for `derive_extreme_event()` and
#' `derive_vars_extreme_event()`.
#'
#' Find the first available record from `events` for each by group.
#' All variables of the selected observation are kept.  These can be kept if
#' added as new observations or dropped if merged to the source data. It can be
...
#'   filter(PARAMCD == "CBOR")
#'
derive_extreme <- function(dataset,
                           by_vars = NULL,
                           events,
...
#' @keywords der_adsl
#'
#' @seealso [event()], [event_joined()], [derive_extreme()], [derive_extreme_event()]
#'
#' @export
#'
#' @examples
