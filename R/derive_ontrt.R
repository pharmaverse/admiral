...
#'
#' Determine if a single date is collected on-treatment. May be used in the
#' derivation of `ONTRTFL`.
#'
#' @param dataset `data.frame`.
#'
#' @param new_var Name of variable to store computed flag
#'
...
#'   Required; A date or date-time object column is expected.
#'
#' @param ref_end_present String value of `ref_end_date` parameter.
#'
...
derive_ontrt <- function(dataset,
                         ...
                         ref_end_date,
                         ref_end_window,
                         ref_end_window_units,
                         timepoint_var,
                         start_timepoint_pre_value) {
  warn_if_vars_exist(
    dataset,
  ...
  if (ref_end_present == "") {
    ...
    dataset <- dataset %>%
      mutate(TEMP_REF_END_DATE = compute_ontrt_date_win(!!enquo(ref_end_date),
                                                        ref_end_window,
                                                        ref_end_window_units))
  }
  ...
  dataset <- dataset %>%
    mutate(!!enquo(new_var) :=
             case_when(is.na(!!enquo(start_date)) == TRUE &
                         is.na(!!enquo(ref_start_date)) == FALSE ~ "Y",
                       is.na(!!enquo(timepoint_var)) == TRUE &
                         is.na(!!enquo(start_date)) == FALSE &
                         is.na(!!enquo(ref_start_date)) == FALSE &
                         !!enquo(ref_start_date) == !!enquo(start_date) ~ "Y",
                       is.na(!!enquo(timepoint_var)) == FALSE &
                         is.na(!!enquo(start_date)) == FALSE &
                         is.na(!!enquo(ref_start_date)) == FALSE &
                         !!enquo(ref_start_date) == !!enquo(start_date) &
                         !!enquo(timepoint_var) %!in% !!start_timepoint_pre_value
                       ~ "Y"))

  if (ref_end_present == "") {
    dataset <- dataset %>%
      mutate(!!enquo(new_var) :=
               ifelse(is.na(!!enquo(ref_start_date)) == FALSE &
                        !!enquo(ref_start_date) <
                        !!enquo(start_date), "Y", !!enquo(new_var)))
  } else {
    dataset <- dataset %>%
      mutate(!!enquo(new_var) :=
               ifelse(is.na(!!enquo(ref_start_date)) == FALSE &
                        ...
                        !!enquo(start_date) <= TEMP_REF_END_DATE, "Y",
                      !!enquo(new_var)))
  }
