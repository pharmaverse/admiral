...
#' Is a list named and with correct length?
#'
#' Checks if a list is a named list and whether it has requested length
#'
#' @param arg A list to be checked.
#' @param optional Is the list optional? If set to `FALSE` and `arg`
#'   is `NULL` then an error is thrown.
#' @param min_len Minimum length of the list. Set to `NULL` (default) if not restricted.
#' @param max_len Maximum length of the list. Set to `NULL` (default) if not restricted.
#'
#' @author Ondrej Slama
#'
#' @return The function throws an error if:
#' * `arg` is not a list,
#' * `names(arg)` is `NULL`
#' * any of the list names is empty string (i.e. equal to `""`)
#' * `min_len` / `max_len` is not `NULL` and `arg` does not have the required length
#'
#' @export
#'
#' @keywords assertion
#'
#' @examples
#' assert_named_list(list(a = 1))
#' \dontrun{
#' assert_named_list(list(1))
#' assert_named_list(list(a = 1), min_len = 2)
#' }
assert_named_list <- function(arg, optional = FALSE, min_len = NULL, max_len = NULL) {
  assert_logical_scalar(optional)
  stopifnot(
    is.null(min_len) || is_integerish(min_len),
    is.null(max_len) || is_integerish(max_len)
  )
  ...
  if (!is.list(arg) || is.null(names(arg)) || any(names(arg) == "")) {
    err_msg <- sprintf(
      "`%s` must be a named list but is %s",
      arg_name(substitute(arg)),
      what_is_it(arg)
    )
    abort(err_msg)
  ...
  if (!is.null(min_len) && length(arg) < min_len) {
    err_msg <- sprintf(
      "`%s` must have minimal length %d but has %d",
      arg_name(substitute(arg)),
      min_len,
      length(arg)
    )
  ...
  if (!is.null(max_len) && length(arg) > max_len) {
    err_msg <- sprintf(
      "`%s` must have maximal length %d but has %d",
      arg_name(substitute(arg)),
      max_len,
      length(arg)
    )
